...
%MMFMEXACTITERATION Finds a heuristic policy for the MMFM problem that has
%the same control limit in each state.
...
numStates = length(rates);
    ...
    function [policy_next, v_next] =  UniformIterate(policy_prev, v_prev)
       ...
       %(c+a+v_prev)*integral(@(x) PDF(x).*MmfmExpectedDiscount( discountGenerator, Inf(numStates,1), x, 1, 1:numStates ),0,policy_next(1)) + (c+v_prev)*(1-CDF(policy_next(1)))*MmfmExpectedDiscount( discountGenerator, Inf(numStates,1), policy_next(1), 1, 1:numStates );
