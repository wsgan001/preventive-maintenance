function [ MLE ] = JumpQuantityMLE( data, from,to, lambda, k, numExperiments )
...
MLE=@(rates,jumpQuantities) -sum(arrayfun(@(n) data{n,2}(from,to).*expressions{n,1}(rates,jumpQuantities),dummy));
end
