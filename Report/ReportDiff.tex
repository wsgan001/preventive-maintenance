...
\newcolumntype{L}{>{\raggedright\arraybackslash}X} %DIF > 
\usepackage{ltablex} %DIF > 
...
%DIF -------
\newtheorem{theorem}{Theorem}[chapter] %DIF > 
%DIF -------

%DIF < \newtheorem{lemma}{Lemma} 
%DIF -------
\newtheorem{lemma}{Lemma}[chapter]  %DIF > 
...
\begin{document}
	...
	\begin{abstract}
		...
		We assume the cost of correctively repairing the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend is larger than the cost of preventively repairing it.
		...
		\DIFaddbegin \DIFadd{A method to estimate parameters of the MMFM from usage data of the asset is also presented.
		}\DIFaddend \end{abstract}
	
	\chapter*{Executive summary}
	\DIFdelbegin %DIFDELCMD < [%%%
	\DIFdel{Describe characteristics of a machine whose deterioration that can be }\DIFdelend \DIFaddbegin \DIFadd{Often, the defect of an asset comes with additional costs.
	...
	\DIFadd{In this thesis, a method is derived that finds the optimal times to perform preventive maintenance.
		The degradation of the asset was }\DIFaddend modeled by a \DIFdelbegin \DIFdel{MMFM with jumps:
		different activities that in different degrees wear out the machine, the machine has no schedule (actions as CTMC), partial repairs between activities and an initial fitness of the machine.}%DIFDELCMD < ]
	%DIFDELCMD < %%%
	\DIFdelend \DIFaddbegin \DIFadd{Markov modulated fluid model.
		This corresponds to an asset with the following characteristics:
	}\begin{itemize}
		\item \DIFadd{There is a distinct set of activities that the asset is used for.
		}\item \DIFadd{The usage of the asset is recorded; at each time (in the past) it is known for what activity it is used.
		}\item \DIFadd{No schedule is known; it is not certain for which activity the asset will be used in the future.
		}\item \DIFadd{Maintenance restores the asset to a condition as good as new.
		}\item \DIFadd{When the asset is not maintained, the condition deteriorates over time (Negative aging).
			This is in contrary to assets that get more reliable over time.
		}\item \DIFadd{Optionally, the speed at which the asset wears out, depends on the activity it is being used for.
		}\item \DIFadd{Optionally, between certain (fixed) activities, the condition of the asset improves.
			For instance, when each time before a certain activity, the asset is inspected and partial maintenance is performed.
	}\end{itemize}
	\DIFaddend 
	
	\DIFdelbegin %DIFDELCMD < [%%%
	\DIFdel{List assumptions}%DIFDELCMD < ]
	%DIFDELCMD < %%%
	\DIFdelend \DIFaddbegin \DIFadd{In order to use the presented method, certain parameters of the degradation model are needed.
	...
	\DIFdelbegin %DIFDELCMD < [%%%
	\DIFdel{The type of observations for which the policy is useful: trace data where at each time the current activityis known and failures are observed.
	}%DIFDELCMD < ]
	%DIFDELCMD < %%%
	\DIFdelend \DIFaddbegin \DIFadd{We present the following approach to determine the times at which preventive maintenance should be performed:
	}\begin{itemize}
		\item \DIFadd{First, control limits for the various activities must be calculated.
		...
		}\item \DIFadd{When using the asset, data from its usage since the last repair are used to determine whether the asset should be repaired at each moment.
	}\end{itemize}
	\DIFaddend 
	...
	\DIFdelbegin %DIFDELCMD < [%%%
	\DIFdel{Mention the kind of historical data needed for the parameter estimation.
	}%DIFDELCMD < ]
	%DIFDELCMD < 
	
	%DIFDELCMD < [%%%
	\DIFdel{The type of (online) policy that is presented and how it would be implemented.
	}%DIFDELCMD < ]
	%DIFDELCMD < 
	
	%DIFDELCMD < [%%%
	\DIFdel{Explain possibilities of forecasting time until preventive repair.}%DIFDELCMD < ]
	%DIFDELCMD < 
	...
	\DIFaddend \chapter{Introduction}\label{chapter:Introduction}
	...
	Preventive maintenance problems can be classified based on various aspects\DIFdelbegin \DIFdel{:
	}\DIFdelend \DIFaddbegin \DIFadd{.
	}\DIFaddend First of all, there is the distinction between perfect and imperfect maintenance.
	...
	We will \DIFdelbegin \DIFdel{first }\DIFdelend explain some useful results and concepts from \DIFaddbegin \DIFadd{the fields of }\DIFaddend Markov decision theory \DIFdelbegin \DIFdel{.
		After that, we will do the same for }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend survival analysis.
	...
	\DIFdelbegin \DIFdel{There are various types of Markov Decision Problems (MDPs).
		A distinction can be made based on the time horizon of the problem; whether it is finite orinfinite.
		For infinite horizon problems, the total cost might be infinite.
		Two approaches are common to resolve this issue of infinity: The first is to discount costs further in the future; The second is to consider the }\DIFdelend \DIFaddbegin \DIFadd{In a Markov Decision Process (MDP), the evolution of the process depends on the chosen control action.
		The objective of an MDP is usually to minimize costs (or, equivalently, maximize profit) in some sense.
	}
	
	\DIFadd{For a discrete MDP with state space $X$, when at the $k$'th decision epoch the process is in state $x_k\in X$, an action must be chosen out of a set $U(x_k)$ of allowed actions.
		...
		Often, this is done by either considering the long-run }\DIFaddend average cost per \DIFdelbegin \DIFdel{time unit in }\DIFdelend \DIFaddbegin \DIFadd{decision epoch, or by multiplying costs in decision epoch $k$ by $\alpha^k$, where $0<\alpha<1$ is a discount factor.
		The latter cost is called the discounted cost.
		In this thesis we will consider discounted costs.
		This has the advantage over long-run average costs that }\DIFaddend the \DIFdelbegin \DIFdel{long run.
		We will first compare these two approaches.
		After that we will explain the structure of }\DIFdelend \DIFaddbegin \DIFadd{Bellman equations can be to solve the problem.
	}
	
	\DIFadd{Admissible }\DIFaddend solutions to MDPs \DIFdelbegin \DIFdel{and discuss methods to find them.}\subsubsection{\DIFdel{Discounted vs. long run average cost}}
	%DIFAUXCMD
	\addtocounter{subsubsection}{-1}%DIFAUXCMD
	%DIFDELCMD < [%%%
	\DIFdel{Explain relation and pros and cons of both}%DIFDELCMD < ]
	%DIFDELCMD < %%%
	\subsubsection{\DIFdel{Stationary policies}}
	%DIFAUXCMD
	\addtocounter{subsubsection}{-1}%DIFAUXCMD
	\section{\DIFdel{Survival analysis}}
	%DIFAUXCMD
	\addtocounter{section}{-1}%DIFAUXCMD
	\subsection{\DIFdel{Classification of lifetime distributions}}
	%DIFAUXCMD
	\addtocounter{subsection}{-1}%DIFAUXCMD
	%DIFDELCMD < [%%%
	\DIFdel{increasing and decreasing hazard rate.
		No preventive maintenance for decreasing hazard rates.
	}%DIFDELCMD < ]
	%DIFDELCMD < %%%
	\DIFdelend \DIFaddbegin \DIFadd{are policies that choose an allowed action for each decision epoch and state, i.e. for each $k$ there is some $\mu_k$ with $\mu_k(x_k)\in U(x_k)$ for all $k,x_k$.
		...
		Section 6.2.4 of \mbox{%DIFAUXCMD
			\cite{Puterman2008} }%DIFAUXCMD
		proves that for discounted infinite-horizon countable-state problems with cost-function $g$ independent of decision epoch $k$, there exists an optimal stationary policy.
	}\DIFaddend 
	
	\DIFaddbegin \DIFadd{In this thesis, we will aim at minimizing the total discounted cost for an infinite-horizon MDP, i.e. minimizing
	}\[
	\DIFadd{\mathbb{E}\left[\sum\limits_{k=0}^\infty \alpha^k g(x_k,\mu(x_k))\right],
	}\]
	\DIFadd{for some stationary policy $\mu$. 
	}
	
	
	\DIFaddend \chapter{Age-based maintenance}\label{chapter:AgeBased}
	\DIFdelbegin \DIFdel{A machine }\DIFdelend \DIFaddbegin \DIFadd{An asset }\DIFaddend is considered that is subject to deterioration over time.
	If no further observations are made\DIFdelbegin \DIFdel{while the machine is active}\DIFdelend , any choice to repair the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend can only be based on its age.
	...
	In this section, the problem of choosing when to repair the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend is defined as a Markov decision process.
	\DIFdelbegin \DIFdel{We consider a machine that is subject to deterioration over time.
	}\DIFdelend Calendar time is discretized in steps of size $\delta$, i.e. the $k$'th decision \DIFdelbegin \DIFdel{stage }\DIFdelend \DIFaddbegin \DIFadd{epoch }\DIFaddend is at time $t_k=k\delta$.
	We refer to the interval $(k\delta,(k+1)\delta]$ as the $k$'th time interval.
	At \DIFdelbegin \DIFdel{stage }\DIFdelend \DIFaddbegin \DIFadd{epoch }\DIFaddend $k\in\mathbb{N}\cup\{0\}$, we denote the state of the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend by $x_k\in X=\mathbb{N}_0$ and initially $x_0=1$.
	When $x_k=x$, this means that after the $k$'th time interval, the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend has age $x\delta$.
	
	The lifetime of the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend has a distribution function $F$.
	We denote the reliability function by $\bar F(x):=1-F(x)$ \DIFdelbegin \DIFdel{, the }\DIFdelend \DIFaddbegin \DIFadd{and the conditional reliability function by $\bar F(x;y)=\frac{\bar F(x)}{\bar F(y)}$.
		We assume that this distribution function is continuously differentiable so that the }\DIFaddend probability density function by $f$ and the hazard rate 
	\DIFdelbegin \DIFdel{by 
	}\DIFdelend \[
	h(x)=\frac{f(x)}{\bar F(x)}
	\DIFdelbegin \DIFdel{.
	}\DIFdelend \] 
	\DIFaddbegin \DIFadd{are both continuous.
	}\DIFaddend 
	
	\subsection{\DIFdelbegin \DIFdel{Stochastic machine breakdown}\DIFdelend \DIFaddbegin \DIFadd{Degradation model}\DIFaddend }
	To model the \DIFdelbegin \DIFdel{breaking of the machine}\DIFdelend \DIFaddbegin \DIFadd{degradation of the asset}\DIFaddend , we introduce random variables $\omega_k:=\omega_k(x_k)$ at decision epoch $k$ which only depend on $x_k$.
	$$
	\omega_k(x_k):=\DIFdelbegin %DIFDELCMD < \begin{cases}
	%DIFDELCMD < 1,&\text{if the machine will reach age }\ \delta x_{k}\text{ given}\\
	%DIFDELCMD < &\text{that had reached age}\ \delta(x_k-1)\ \text{ at time } t_k \\
	...
	\end{cases}
	\DIFaddend $$
	If the lifetimes of the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend are i.i.d. random variables with distribution $F$ then for $x>0$
	$$
	\mathbb{P}(\omega_k(x_k)=1)=\frac{1-F(\delta x_k)}{1-F(\delta (x_k-1))}.
	$$
	\DIFdelbegin \DIFdel{In the rest of the thesis we assume that the lifetime of the machine has an increasing hazard rate so that $\mathbb{P}(\omega_k(x)=1)$ is decreasing in $x$.
	}\DIFdelend 
	
	\subsection{Control actions}
	At the $k$'th decision \DIFdelbegin \DIFdel{stage}\DIFdelend \DIFaddbegin \DIFadd{epoch}\DIFaddend , we shall choose an action $u_k$ from the action set $U(x_k)$.
	Where
	$$
	U(x_k):=\begin{cases}
	...
	\begin{itemize}
		\item $a_R$:
		Repair (or replace) the \DIFdelbegin \DIFdel{machine}\DIFdelend \DIFaddbegin \DIFadd{asset}\DIFaddend .
		\item $a_W$:
		Do nothing.
	\end{itemize}
	...
	\begin{itemize}
		\item If the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend is repaired, its age will be $\delta$ at the next stage.
		\item If the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend fails, its age will be $0$ at the next stage.
		\item If the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend does not fail and no repair is done, the age of the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend will increase by $\delta$.
	\end{itemize}
	...
	\end{cases}
	$$
	For convenience, we define the random variable $S(x_k):=f(x_k,a_W,\omega_k(x_k))$ to denote the age of the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend one time interval after it was $x_k$.
	Note that in the above definition, repairing the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend takes exactly one time interval.
	
	\subsection{Costs and discounting}
	Preventively repairing the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend has a cost $c>0$.
	When the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend needs to be repaired correctively, an additional cost $a>0$ \DIFdelbegin \DIFdel{also }\DIFdelend needs to be paid.
	Hence, when the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{process }\DIFaddend is in state $x_k$ and the action $u_k$ is chosen, the following cost is incurred
	$$
	...
	As \DIFdelbegin \DIFdel{will be proven in section \ref{section:AgeBasedStationaryPolicy}, the optimal policy is a stationary policy}\DIFdelend \DIFaddbegin \DIFadd{the problem as defined above is a Markov decision process with $g$ independent of the decision epoch, there exists an optimal stationary policy \mbox{%DIFAUXCMD
			\cite{Puterman2008}}%DIFAUXCMD
	}\DIFaddend .
	Hence, we want to find a stationary policy $\mu:X\rightarrow \{a_W,a_R\}$ that chooses the action $u_k=\mu(x_k)$ that minimizes the expected total discounted cost \DIFdelbegin \DIFdel{$V_\delta(x_0,\mu;0)$}\DIFdelend \DIFaddbegin \DIFadd{$V_\delta(x_0,\mu)$}\DIFaddend .
	For a policy $\mu$, \DIFdelbegin \DIFdel{$V_\delta(x_k,\mu;k)$ }\DIFdelend \DIFaddbegin \DIFadd{$V_\delta(x_k,\mu)$ }\DIFaddend is given by
	$$
	V_\delta(x_k,\mu\DIFdelbegin \DIFdel{;k}\DIFdelend )=g(x_k,\mu(x_k))+\alpha_\delta \DIFdelbegin \DIFdel{\mathbb{E}[V_\delta(f(x_k,\mu(x_k),\omega_k(x_k)),\mu;k+1)]}\DIFdelend \DIFaddbegin \DIFadd{\mathbb{E}[V_\delta(f(x_k,\mu(x_k),\omega_k(x_k)),\mu)]}\DIFaddend .
	$$
	The Bellman equations for the optimal cost $V^*_\delta$ read
	\begin{equation}\label{eq:AgeBasedBellman}
	V^*_\delta(x_k\DIFdelbegin \DIFdel{;k}\DIFdelend )=\DIFdelbegin %DIFDELCMD < \begin{cases}
	%DIFDELCMD < \min\{c+\alpha_\delta V^*_\delta(1;k+1),\alpha_\delta \mathbb{E}[V^*_\delta(S(x_k));k+1]\},&\text{if}\ x_k>0 \\
	%DIFDELCMD < c+a+\alpha_\delta V^*_\delta(1;k+1),&\text{else.}
	%DIFDELCMD < \end{cases}
	%DIFDELCMD < %%%
	\DIFdelend \DIFaddbegin \begin{cases}
	...
	\DIFdelbegin \DIFdel{For an infinite horizon, the Bellman equations only depend on the state $x_k$ and not on the decision epoch $k$.
		This means that the optimal cost also does not depend on the decision epoch.
		Hence, it will }\DIFdelend \DIFaddbegin \DIFadd{Under the optimal policy, the total discounted cost $V_\delta(x_k,\mu;k)=V^*_\delta(x_k)$ for all $x_k,k$.
		Since we only consider stationary policies, the $k$ will also }\DIFaddend be suppressed in the notation \DIFaddbegin \DIFadd{of $V_\delta(x_k,\mu)$.
	}
	
	\subsection{\DIFadd{Continuous-time MDP}}
	\DIFadd{In the remainder of the chapter, we will consider a continuous-time MDP that is obtained by letting $\delta\rightarrow0$.
	...
	\DIFadd{\alpha_\delta=1-\delta\beta+o(\delta^2).
	}\]
	\DIFadd{When action $a_W$ is chosen, at age $x_k$, the transition probabilities become
	}$$
	\DIFadd{\mathbb{P}(\omega_k(x_k)=1)=\frac{1-F(\delta x_k)}{1-F(\delta (x_k-1))}=1-\delta h(\delta x_k)+o(\delta^2),
	}$$
	\DIFadd{and
	}\[
	\DIFadd{\mathbb{P}(\omega_k(x_k)=0)=\frac{F(\delta x_k)-F(\delta (x_k-1))}{1-F(\delta (x_k-1))}=\delta h(\delta x_k)+o(\delta^2).
	}\]
	\DIFadd{This hazard rate $h$ is assumed to be increasing.
		...
		Note that we have to write $V^*(0^+)$ instead of $V^*(0)$ for a new asset as the latter corresponds to the asset being broken}\DIFaddend .
	\DIFdelbegin \DIFdel{Under the optimal policy, the total discounted cost $V_\delta(x_k,\mu)=V^*_\delta(x_k)$ for all $x_k,k$.
	}\DIFdelend 
	...
	\end{cases}
	$$
	This would however, introduce the possibility of having to correctively repair the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend twice in a row.
	\DIFdelbegin \DIFdel{Furthermore}\DIFdelend \DIFaddbegin \DIFadd{However}\DIFaddend , when we let $\delta\rightarrow 0$, this would not make any difference.
	...
	\DIFdelbegin \DIFdel{Referring back to the Bellman equations of the }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
			\cite{Puterman2008} }%DIFAUXCMD
		proves that for a discounted infinite-horizon countable-state MDP where the cost function does not depend on the decision stage, there exists a stationary policy that is optimal.
		This proves that there exists an optimal stationary policy to this }\DIFaddend age-based \DIFdelbegin \DIFdel{problem }%DIFDELCMD < \eqref{eq:AgeBasedBellman}%%%
	\DIFdel{, you can see that when $x_k>0$, repair is chosen whenever
	}\[ \DIFdel{c+\alpha_\delta V^*_\delta(1) <\alpha_\delta \mathbb{E}[V^*_\delta(S(x_k))]. }\]
	%DIFAUXCMD
	\DIFdel{Where the left hand side is a constant and the right hand side only depends on $S(x_k)$, which only depends on the age of the machine by assumption.
		Hence, we have established that the optimalpolicy only depends on the age $x_k$ so that it must be a stationary policy }\DIFdelend \DIFaddbegin \DIFadd{preventive maintenance policy}\DIFaddend .
	
	\subsection{Control limit}\DIFdelbegin \DIFdel{Using similar reasoning, for states $x>0$}\DIFdelend \DIFaddbegin \label{section:AgeBasedControlLimit}
	\DIFadd{From the Bellman equations }\eqref{eq:AgeBasedBellman} \DIFadd{we can see that, by letting $\delta\rightarrow 0$}\DIFaddend , repair is \DIFdelbegin \DIFdel{chosen whenever
	}\DIFdelend \DIFaddbegin \DIFadd{for chosen $x>0$ whenever
	}\DIFaddend \begin{equation}\label{eq:AgeBasedRepairCondition}
	c+\alpha \DIFdelbegin \DIFdel{_\delta }\DIFdelend V^*\DIFdelbegin \DIFdel{_\delta(1) <\alpha_\delta \mathbb{E}[V^*_\delta(S(x))]}\DIFdelend \DIFaddbegin \DIFadd{(0^+) \leq V^*(x)}\DIFaddend . 
	\end{equation}
	...
	\begin{enumerate}
		...
		In this case we set control limit \DIFdelbegin \DIFdel{$\mu=\infty$}\DIFdelend \DIFaddbegin \DIFadd{$\mu^*=\infty$}\DIFaddend .
		...
		What happens for ages greater than this \DIFdelbegin \DIFdel{$\mu$ }\DIFdelend \DIFaddbegin \DIFadd{$\mu^*$ }\DIFaddend is not relevant as these will never be reached.
	\end{enumerate}
	\DIFaddbegin \DIFadd{Later in this chapter, we will prove that the right hand side of }\eqref{eq:AgeBasedRepairCondition} \DIFadd{is increasing for $x<\mu^*$.
	}\DIFaddend Hence, we have established that the optimal policy must be of control limit type where the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend is repaired whenever its age exceeds some threshold \DIFdelbegin \DIFdel{$\mu$}\DIFdelend \DIFaddbegin \DIFadd{$\mu^*$}\DIFaddend . \section{Computation of total discounted cost}
	In this section, the Bellman equations will be used to find the expected total discounted cost of the optimal cost.
	\DIFaddbegin \DIFadd{From the discrete Bellman equations }\eqref{eq:AgeBasedBellman}\DIFadd{, differential equations will be derived.
	}\DIFaddend The following Bellman equations will be considered
	...
	\DIFdel{V^*(x):=\lim\limits_{\delta\rightarrow 0} V^*_\delta(x).
	}\end{displaymath}
	%DIFAUXCMD
	\DIFdel{(Note that we needed to define $V^*(0^+)$ in this way since $V^*(0^+)=V^*(0)-c-a$).
	}\DIFdelend \begin{remark}\label{remark:AgeBasedWeirdODE}
		The differential equation \eqref{eq:AgeBasedBellmanODE} seems counterintuitive as for high hazard rates, the expected total discounted cost would be decreasing for increasing age of the \DIFdelbegin \DIFdel{machine}\DIFdelend \DIFaddbegin \DIFadd{asset}\DIFaddend .
	...
	\begin{theorem}
		When the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend has age $x$, the expected remaining total discounted cost equals
	...
	\end{theorem}
	Unfortunately, the value of $V^*(0^+)$ depends on the \DIFdelbegin \DIFdel{policy that is chosen and it seems }\DIFdelend \DIFaddbegin \DIFadd{optimal policy and it is also }\DIFaddend difficult to solve $V^*(x)=c+V(0^+)$ analytically for $x$.
	Control limit $\mu^*$ is then chosen as the smallest positive $x$ that satisfies $V(x)=c+V(0^+)$ if such $x$ exist and $\mu^*=\infty$ else.
	The policy that we just derived, schedules preventive maintenance at time $\mu^*$ if the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend has not already failed by then.
	...
	The length of one run of the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend is the minimum of its lifetime \DIFdelbegin \DIFdel{$Q_0$ }\DIFdelend \DIFaddbegin \DIFadd{$Q_0\sim F$ }\DIFaddend and the chosen control limit $\mu$.
	At the end of each run, at least $c$ is paid.
	If the run ends because the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend broke (i.e. $Q_0<\mu$), an additional cost of $a$ is paid.
	...
	\end{equation}
	\DIFaddbegin \DIFadd{Where $A\wedge B$ denotes the minimum af $A$ and $B$.
	}\DIFaddend We get a similar expression for $V(x,\mu)$:
	
	\begin{theorem}
		Given control limit $\mu$ and \DIFdelbegin \DIFdel{a machine }\DIFdelend \DIFaddbegin \DIFadd{an asset }\DIFaddend of age $x$, the expected remaining total cost equals 
	...
	\begin{remark}
		In theorem \ref{theorem:TdcNonDecreasing}, we will prove that for increasing hazard rates, $V(x,\mu)$ is \DIFdelbegin \DIFdel{nondecreasing in $x$}\DIFdelend \DIFaddbegin \DIFadd{increasing for $x<\mu$}\DIFaddend .
	\end{remark}
	...
	\begin{example}
		Let \DIFdelbegin \DIFdel{$Q\sim\text{Exp}(\lambda)$}\DIFdelend \DIFaddbegin \DIFadd{$Q_0\sim\text{Exp}(\lambda)$}\DIFaddend .
		...
		\DIFdel{\frac{d}{dx}V(x,\mu^*)=0=-\lambda(c+a+ V(0^+,\mu^*))+(\beta+ \lambda) V(x,\mu^*)=-\lambda(c+a+ V(0^+,\mu^*))+(\beta+ \lambda) V(0^+,\mu^*).
		}\end{displaymath}
		%DIFAUXCMD
		\DIFdel{Which results in
		...
		\DIFadd{Where the last equality holds as $V(x,\mu^*)$ is constant.
	...
	This corresponds to minimizing the expected total discounted cost for \DIFdelbegin \DIFdel{a machine }\DIFdelend \DIFaddbegin \DIFadd{an asset }\DIFaddend using control limit $\mu$ in the first run and optimal control limit $\mu^*$ afterwards.
	...
	%DIFDELCMD < &F(\mu)\mathbb{E}[e^{-\beta Q_0}|Q_0\leq \mu]=\int\limits_0^\mu f(x)e^{-\beta x}dx\\
	%DIFDELCMD < &\Rightarrow \frac{d}{d\mu}\left[F(\mu)\mathbb{E}[e^{-\beta Q_0}|Q_0\leq \mu]\right]=f(\mu)e^{-\beta \mu}.
	%DIFDELCMD < \end{split}
	%DIFDELCMD < %%%
	...
	Consider the cost of one run of the \DIFdelbegin \DIFdel{machine}\DIFdelend \DIFaddbegin \DIFadd{asset}\DIFaddend , we replace the repair cost by $c^*=c+V(0^+,\mu^*)$ so that the expected discounted cost of the first repair equals the expected total discounted cost of the original problem.
	$V(x,\mu^*)$ now corresponds to the cost of this altered problem, but starting with \DIFdelbegin \DIFdel{a machine }\DIFdelend \DIFaddbegin \DIFadd{an asset }\DIFaddend of age $x$.
	If $V(x,\mu^*)$ were to be decreasing in the neighborhood of some $x$, this would mean that for that $x$, the problem would have a lower expected optimal cost if we started with a slightly older \DIFdelbegin \DIFdel{machine}\DIFdelend \DIFaddbegin \DIFadd{asset}\DIFaddend .
	This seems to conflict with the assumption that $h$ is increasing so that the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend deteriorates over time.
	...
	\begin{theorem}\label{theorem:TdcNonDecreasing}
		...
		\begin{proof}
			...
			\]
			\DIFaddbegin \DIFadd{Note that, in order to take the derivative of $h$, we have to assume that $h$ (or equivalently $f$) is differentiable.
			}\DIFaddend We know that for our $x'$, $\frac{d}{dx}V(x,\mu^*)\leq 0$ and $V(x,\mu^*)\leq c+V(0^+,\mu^*)$.
			Furthermore, by assumption $h'(x')>0$.
			Concluding
			\[
			\frac{d^2}{dx^2}V(x',\mu^*)<0.
			\]
			This implies that if \DIFdelbegin \DIFdel{$\frac{d}{dx}V(x',\mu^*)=\leq0$ }\DIFdelend \DIFaddbegin \DIFadd{$\frac{d}{dx}V(x',\mu^*)\leq0$ }\DIFaddend for some $x'$, $V(x,\mu^*)$ is \DIFdelbegin \DIFdel{strictly }\DIFdelend concave down after $x'$ so that for all $x>x'$
			\[
			V(x,\mu^*)< c+V(0^+,\mu^*).
			\]
			Hence no control limit will ever be \DIFaddbegin \DIFadd{reached }\DIFaddend and $V(x,\mu^*)$ will eventually become negative.
	...
	\begin{corollary}
		...
		\end{proof}
	\end{corollary} \section{\DIFdelbegin \DIFdel{Computing the optimal policy}\DIFdelend \DIFaddbegin \DIFadd{Policy iteration}\DIFaddend }\label{section:AgeBasedOptimalPolicyComputation}
	...
	In this section we propose a \DIFdelbegin \DIFdel{method of successive approximation of the }\DIFdelend \DIFaddbegin \DIFadd{policy iteration method to find the optimal }\DIFaddend control limit and the total discounted cost.
	...
	\begin{remark}
		...
		This is expected, as this would decrease the incentive to prevent the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend from failing.
	\end{remark}
	\begin{remark}
		Decreasing $c$ would result in a decrease of the hazard bound, decreasing the control limit.
		This is also \DIFdelbegin \DIFdel{expected }\DIFdelend \DIFaddbegin \DIFadd{natural }\DIFaddend as this would make $a$ relatively larger.
	\end{remark}
	\begin{remark}
	...
	\begin{remark}\label{remark:AgeBasedControlLimitDiscountIncrease}
		...
		This can be explained by the fact that with a higher discount, costs further in the future weigh less \DIFdelbegin \DIFdel{such }\DIFdelend \DIFaddbegin \DIFadd{so }\DIFaddend that you would want to move costs as far into the future as possible.
	\end{remark}
	\begin{remark}
	...
	\begin{remark}\label{remark:AgeBasedControlLimitDiscountAndHazardIncrease}
		If the hazard rate and the \DIFdelbegin \DIFdel{control limit }\DIFdelend \DIFaddbegin \DIFadd{discount exponent }\DIFaddend are both multiplied by a constant, then the hazard rate remains the same as this constant can just be divided out of this equation.
	\end{remark}
	...
	\begin{remark}\label{remark:AgeBasedCostRatio}
		...
		This is natural as \DIFdelbegin \DIFdel{expressing the valuta }\DIFdelend \DIFaddbegin \DIFadd{changing the currency }\DIFaddend the costs are expressed in should not be relevant.
	\end{remark}
	\begin{remark}
		...
		This is natural as \DIFdelbegin \DIFdel{a machine that fails }\DIFdelend \DIFaddbegin \DIFadd{an asset that breaks }\DIFaddend quicker is more expensive to maintain.
	\end{remark}
	...
	Initially, the bucket has a random amount of fluid \DIFdelbegin \DIFdel{$Q\sim F$}\DIFdelend \DIFaddbegin \DIFadd{$Q_0\sim F$}\DIFaddend .
	The fluid decreases \DIFdelbegin \DIFdel{constantantly }\DIFdelend \DIFaddbegin \DIFadd{constantly }\DIFaddend with rate $1$ and no fluid jumps occur.
	...
	In the real world, these fluid jumps could correspond to a partial repair of the \DIFdelbegin \DIFdel{machine}\DIFdelend \DIFaddbegin \DIFadd{asset}\DIFaddend .
	The presence of jumps introduces the following complications:
	\begin{itemize}
		\item The hazard of the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend failing at some time $t$ cannot be derived directly from the age only but also depends on the number of jumps that occurred before $t$.
		\item Furthermore, the times at which these jumps occurred also matter.
		When a jump occurs at time $t$, it is certain that the fluid quantity is at least $J$ so that you know for certain that in the interval $[t,t+J)$ the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend cannot fail.
	\end{itemize}
	...
	First we define the underlying stochastic process of the \DIFdelbegin \DIFdel{state of the machine deteriorating }\DIFdelend \DIFaddbegin \DIFadd{the asset degrading }\DIFaddend over time and instantaneously increasing at the occurrence of jumps.
	After that, we define a Markov decision process similarly to section \ref{section:AgeBasedDefinition}.
	
	\subsection{Stochastic \DIFdelbegin \DIFdel{machine breakdown}\DIFdelend \DIFaddbegin \DIFadd{asset degradation}\DIFaddend }
	...
	\DIFaddbegin \DIFadd{The fluid process is absorbing at $Q(t)=0$.
	}\DIFaddend Hence, let $P_\lambda(t_1,t_2)$ be a Poisson distributed random variable with rate $\lambda (t_2-t_1)$.
	\DIFdelbegin \DIFdel{The fluid process is absorbing at $Q(t)=0$.
	}\DIFdelend Then the fluid level at time $t$ is given by 
	\[
	...
	\end{split}
	\]
	The \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend breaks when the fluid level reaches $0$, i.e. at the time $T^*$ given by
	\[
	T^*=\inf\{t|Q(t)=0\}.
	\]
	When the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend is repaired, it starts with an age of zero again.
	...
	This would be a very inconvenient format of the state of the \DIFdelbegin \DIFdel{machine}\DIFdelend \DIFaddbegin \DIFadd{asset}\DIFaddend .
	...
	\begin{example}
		If the first jump of the process would have occur at some time $t$, then \DIFaddbegin \DIFadd{we would know the following at this time $t$}\DIFaddend :
		\begin{enumerate}
			\item \DIFdelbegin \DIFdel{At time $t$, }\DIFdelend $Q(t)\geq J$.
	...
	The \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend cannot break if 
	\[L_c(t)>0,\]
	since we know with certainty that there is still remaining fluid.
	We will refer to this quantity $L_c(t)$ as the fluid buffer.
	The \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend breaks whenever
	\[L_0(t)=Q_0,\]
	i.e. when the initial fluid level is drained.
	Hence, we maintain the two quantities $L_c(t)$ and $L_0(t)$ as the description of the state the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend is in.
	\[
	X(t)=(L_0(t),L_c(t)).
	\]
	Initially
	...
	X(0)=x_{NEW}:=(0,0).
	\]
	\DIFaddbegin \DIFadd{Furthermore, we also define a state $x_{BREAK}$ for when the asset is broken.
		When $X(t)=x_{BREAK}$, $L_0(t)$ and $L_c(t)$ are undefined.
	}\DIFaddend These two quantities evolve in the following way:
	\begin{itemize}
		\item When \DIFdelbegin \DIFdel{$X(t)=(l_0,l_c)$ and }\DIFdelend a jump occurs \DIFaddbegin \DIFadd{at time $t$, }\DIFaddend $L_c$ increases by $J$\DIFdelbegin \DIFdel{.
			Hence, the state changes in the following way
		}\DIFdelend \DIFaddbegin \DIFadd{, i.e.
		}\DIFaddend \begin{equation}\label{eq:SimpleJumpEvolution}
		\DIFaddbegin \DIFadd{X}\DIFaddend (\DIFdelbegin \DIFdel{l_0,l_c}\DIFdelend \DIFaddbegin \DIFadd{t^+}\DIFaddend )\DIFdelbegin %DIFDELCMD < \stackrel{J}{\mapsto} %%%
		\DIFdelend \DIFaddbegin \DIFadd{=}\DIFaddend (\DIFdelbegin \DIFdel{l}\DIFdelend \DIFaddbegin \DIFadd{L}\DIFaddend _0\DIFaddbegin \DIFadd{(t)}\DIFaddend ,\DIFdelbegin \DIFdel{l}\DIFdelend \DIFaddbegin \DIFadd{L}\DIFaddend _c\DIFaddbegin \DIFadd{(t)}\DIFaddend +J).
		\end{equation}
		\item When \DIFdelbegin \DIFdel{the state of the asset at some time }\DIFdelend \DIFaddbegin \DIFadd{no jump occurs in a neighborhood of }\DIFaddend $t$\DIFdelbegin \DIFdel{is given by $X(t)=(l_0,l_c)$ and a time period of length $\tau$ passes without a jump occurring or the machine failing, the fluid level decreases by $\tau$.
			If our buffer contains at least $\tau$ fluid (}\DIFdelend \DIFaddbegin \DIFadd{, $S(t)$ remains constant and $L_0$ and $L_c$ evolve according to
		}\begin{equation}\DIFadd{\label{eq:SimpleAgeEvolution}
			\begin{split}
		...
		\DIFadd{Note that we always have
		}\begin{equation}\DIFadd{\label{eq:SimpleSameDecrease}
			\frac{d}{dt}\left[L_c(t)-L_0(t)\right]=\frac{d}{dt}Q(t)=-1.
		}\end{equation}
		\item \DIFadd{When the fluid drains, }\DIFaddend i.e. \DIFdelbegin \DIFdel{$l_c>\tau$), then the buffer will decrease by $\tau$.
			If our buffer is empty, the drained initial fluid will increase by $\tau$ as we now know with certainty that
			$Q(t)>\tau$ since the machine didn't fail.
		...
		}\begin{displaymath}\DIFdel{%DIFDELCMD < \label{eq:SimpleAgeEvolution}%%%
			(l_0,l_c)\stackrel\tau\mapsto (l_0+\tau-\min\{l_c,\tau\},l_c-\min\{l_c,\tau\}).
		}\end{displaymath}
		%DIFAUXCMD
		%DIFDELCMD < \item %%%
		\item%DIFAUXCMD
		\DIFdel{When the fluid $Q(t)$ reaches zero, it remains zero }\DIFdelend \DIFaddbegin \DIFadd{$Q(t)=0$ or equivalently $L_0(t)=Q_0$, the asset breaks so that
		}\[
		\DIFadd{X(t)=x_{BREAK},
		}\]
		\DIFaddend until it is being repaired.
	\end{itemize}
	\begin{theorem}\label{theorem:SimpleCurrentLevel}
		Using the $L_c(t)$ and the $L_0(t)$ as defined above.
		\DIFdelbegin \DIFdel{The current }\DIFdelend \DIFaddbegin \DIFadd{For $x\neq x_{BREAK}$, the }\DIFaddend fluid level is given by
		...
		\begin{proof}
			...
			When \DIFdelbegin \DIFdel{a time $\tau$ passes without a jump occurring, }\DIFdelend \DIFaddbegin \DIFadd{no jump occurs, we have by }\eqref{eq:SimpleSameDecrease} \DIFadd{that }\DIFaddend $Q(t)$ \DIFdelbegin \DIFdel{decreases by $\tau$.
				From }%DIFDELCMD < \eqref{eq:SimpleAgeEvolution}%%%
			\DIFdel{, we see that $L_c-L_0$ also decreases by $\tau$}\DIFdelend \DIFaddbegin \DIFadd{and $L_c(t)-L_0(t)$ decrease at the same speed}\DIFaddend .
			This completes the proof.
		\end{proof}
	\end{theorem}
	
	\begin{example}\label{example:SimpleFluidState}
		For example, if we have the jump quantity $J=3$ and the initial fluid level for a run of the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend equals $5$.
	...
	\end{example}
	\begin{figure}[H]
		\DIFdelbeginFL %DIFDELCMD < \label{figure:SimpleFluidExampleQuantities}
		%DIFDELCMD < %%%
		...
		\DIFaddbeginFL \label{figure:SimpleFluidExampleQuantities}
	...
	\begin{corollary}\label{corollary:SimpleFluidDistribution}
		...
		\DIFdelbegin %DIFDELCMD < \begin{split}
		%DIFDELCMD < 	F_x(q)&:=\mathbb{P}(Q(t)<q)\\
		%DIFDELCMD < 	&=\mathbb{P}(L_c(t)+Q_0-L_0(t)<q|Q_0>L_0(t))\\
		%DIFDELCMD < 	&=\mathbb{P}(Q_0<q+L_0(t)-L_c(t)|Q_0>L_0(t))\\
		%DIFDELCMD < 	&=\frac{F(q+L_0(t)-L_c(t))-F(L_0(t))}{\bar{F}(L_0(t))}.\\
		%DIFDELCMD < 	\end{split}
		%DIFDELCMD < 	%%%
	...
	%DIFAUXCMD
	\DIFdel{As by }%DIFDELCMD < \eqref{eq:SimpleCurrentLevel}%%%
	\DIFdel{, 
	}\[
	...
	\[
	\omega_k(x_k):=\DIFdelbegin %DIFDELCMD < \begin{cases}
	%DIFDELCMD < \Omega_{SURVIVE}&\ \begin{split}&\text{if the machine does not break and no jump occurs}\\
	%DIFDELCMD < &\text{in the $k$'th time interval.}\end{split}\\
	%DIFDELCMD < \Omega_{BREAK}&\ \text{if the machine breaks in the $k$'th time interval.}\\
	%DIFDELCMD < \Omega_{JUMP}&\ \begin{split}&\text{if the machine does not break and a jump occurs}\\
	%DIFDELCMD < &\text{in the $k$'th time interval.}\end{split}\\
	%DIFDELCMD < \end{cases}
	%DIFDELCMD < %%%
	\DIFdelend \DIFaddbegin \begin{cases}
	...
	\end{split}
	\end{equation}
	\DIFaddbegin \DIFadd{Similar to the previous chapter, we will consider a continuous MDP by letting $\delta\rightarrow0$.
	}\DIFaddend 
	...
	We could also model the problem such that the choice to repair the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend can only be made the instant after a jump occurs.
	This might be more realistic as the jump could be caused by some mechanic performing some partial maintenance and a mechanic might be needed to completely repair the \DIFdelbegin \DIFdel{machine}\DIFdelend \DIFaddbegin \DIFadd{asset}\DIFaddend .
	...
	\DIFdelbegin \DIFdel{From the Bellman equations }%DIFDELCMD < \eqref{eq:SimpleFluidBellman}%%%
	\DIFdel{, it can be seen that when $x_k\neq x_{BREAK}$, repair is chosen when
	}\[ \DIFdel{c+\alpha_\delta V_\delta(x_{NEW},\mu^*) <\alpha_\delta \mathbb{E}[V_\delta(S(x_k),\mu^*)]. }\]
	%DIFAUXCMD
	\DIFdel{Where the left hand side is a constant and the right hand side only depends on $S(x_k)$, which only depends on }\DIFdelend \DIFaddbegin \DIFadd{Any discretization of }\DIFaddend $L_0$ and $L_c$ \DIFdelbegin \DIFdel{and not on $k$ by theorem \ref{theorem:SimpleCurrentLevel}}\DIFdelend \DIFaddbegin \DIFadd{results in a countable state space and the cost function is again independent of the decision epoch}\DIFaddend .
	Hence, \DIFdelbegin \DIFdel{we have established that the optimal policy must again be a stationary }\DIFdelend \DIFaddbegin \DIFadd{by section 6.2.4 of \mbox{%DIFAUXCMD
			\cite{Puterman2008}}%DIFAUXCMD
		, any discrete approximation of the continuous-time MDP has a stationary optimal }\DIFaddend policy.
	...
	\DIFdelbegin \DIFdel{For states $x\neq x_{BREAK}$, repair is chosen whenever
	}\begin{displaymath}\DIFdel{%DIFDELCMD < \label{eq:SimpleFluidRepairCondition}%%%
		c+\alpha_\delta V_\delta(x_{NEW},\mu^*) <\alpha_\delta \mathbb{E}[V_\delta(S(x_k),\mu^*)].
	}\end{displaymath}
	%DIFAUXCMD
	\DIFdelend \DIFaddbegin \DIFadd{The optimal policy is a control limit policy by the same argument as for the age-based problem (see section \ref{section:AgeBasedControlLimit}). 
	}\DIFaddend 
	
	\DIFdelbegin \DIFdel{Now we can distinguish two cases:
	}%DIFDELCMD < \begin{enumerate}
	\begin{enumerate}%DIFAUXCMD
		%DIFDELCMD < 	\item %%%
		\item%DIFAUXCMD
		\DIFdel{There is no value $l_0$ so that $x=(l_0,0)$ satisfies }%DIFDELCMD < \eqref{eq:AgeBasedRepairCondition} %%%
		\DIFdel{and preventive repair is never the optimal choice.
			In this case we set control limit $\mu^*=\infty$.
		}%DIFDELCMD < \item %%%
		\item%DIFAUXCMD
		\DIFdel{There are values $\mu$ so that $x=(\mu,0)$ satisfy }%DIFDELCMD < \eqref{eq:AgeBasedRepairCondition}%%%
		\DIFdel{.
	...
	%DIFDELCMD < %%%
	\DIFdelend Hence, we have established that the optimal policy is a control limit on the used initial fluid $L_0$. \section{Computation of total discounted cost}
	...
	As in the age-based maintenance problem, we \DIFdelbegin \DIFdel{could }\DIFdelend \DIFaddbegin \DIFadd{can }\DIFaddend use the Bellman equations to find differential equations to which the total discounted cost adheres.
	...
	\begin{remark}\DIFdelbegin \DIFdel{label}%DIFDELCMD < {%%%
		\DIFdel{remark:SimpleFluidTDCEquivalence}%DIFDELCMD < }
		%DIFDELCMD < 	%%%
	...
	%DIFDELCMD < \end{enumerate}
	%DIFDELCMD < %%%
	\DIFdel{The first difference can be anticipated using the new formula for the expected total discounted cost }%DIFDELCMD < \eqref{eq:SimpleFluidExplicitTDC}%%%
	\DIFdel{.
		...
		In this way, the control limit will be given by the same equation as for the simple discounted problem }%DIFDELCMD < \eqref{eq:AgeBasedHazardBound}%%%
	...
	\subsection{Stochastic \DIFdelbegin \DIFdel{machine breakdown}\DIFdelend \DIFaddbegin \DIFadd{asset degradation}\DIFaddend }
	First, we define a \DIFaddbegin \DIFadd{left-continuous }\DIFaddend Continuous Time Markov Chain (CTMC) \DIFdelbegin \DIFdel{with states $s_1,...,s_N$}\DIFdelend \DIFaddbegin \DIFadd{$S(t)$ with state space $S=\{s_1,...,s_N\}$}\DIFaddend , initial state $s_1$ and transition rates $\lambda_{ij}$ from state $s_i$ to $s_j$, with $\lambda_{ii}=-\sum_{j\neq i}\lambda_{ij}$.
	...
	\begin{figure}
		\DIFdelbeginFL %DIFDELCMD < \label{figure:MmfmNotation}
		%DIFDELCMD < 	%%%
		\DIFdelendFL \centering
		...
		\DIFaddbeginFL \label{figure:MmfmNotation}
		\DIFaddendFL \end{figure}
	\DIFdelbegin %DIFDELCMD < 
	
	%DIFDELCMD < %%%
	\DIFdel{We define }\DIFdelend \DIFaddbegin \DIFadd{We let }\DIFaddend $S(t)$ \DIFdelbegin \DIFdel{as }\DIFdelend \DIFaddbegin \DIFadd{denote }\DIFaddend the (index of the) state \DIFdelbegin \DIFdel{of }\DIFdelend the CTMC is in at time $t$.
	\DIFdelbegin \DIFdel{Again}\DIFdelend \DIFaddbegin \DIFadd{Similar to the previous chapter}\DIFaddend , we represent the fluid level at time $t$ by $Q(t)$ with $Q(0)=Q_0\sim F$.
	...
	Again, the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend breaks when the fluid reaches zero so that the process is absorbing at $Q(t)=0$.
	\DIFdelbegin \DIFdel{And when it }\DIFdelend \DIFaddbegin \DIFadd{When the asset }\DIFaddend is repaired, the process is restarted \DIFaddbegin \DIFadd{with an initial fluid level with distribution $F$}\DIFaddend .
	...
	X(t)=(S(t),L_0(t),L_c(t)),
	\]
	\DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{when the asset is not broken (i.e. the fluid level is positive) and }\DIFaddend initially $X(0)=x_{NEW}=(1,0,0)$.
	\DIFaddbegin \DIFadd{Again, we define a special state $x_{BREAK}$ for when the asset is broken.
	...
	\begin{itemize}
		\item When \DIFdelbegin \DIFdel{$X(t)=(i,l_0,l_c)$ and a transition occurs to $j$, }\DIFdelend \DIFaddbegin \DIFadd{a transition from $s_i$ to $s_j$ occurs at time $t$, the buffer }\DIFaddend $L_c$ increases by $J_{ij}$\DIFdelbegin \DIFdel{.
			Hence, the state changes in the following way
		}\DIFdelend \DIFaddbegin \DIFadd{:
		}\DIFaddend \begin{equation}\label{eq:MmfmJumpEvolution}
		\DIFaddbegin \DIFadd{X}\DIFaddend (\DIFdelbegin \DIFdel{i,l_0,l_c}\DIFdelend \DIFaddbegin \DIFadd{t^+}\DIFaddend )\DIFdelbegin %DIFDELCMD < \stackrel{J_{ij}}{\mapsto} %%%
		\DIFdelend \DIFaddbegin \DIFadd{=}\DIFaddend (j,\DIFdelbegin \DIFdel{l}\DIFdelend \DIFaddbegin \DIFadd{L}\DIFaddend _0\DIFaddbegin \DIFadd{(t)}\DIFaddend ,\DIFdelbegin \DIFdel{l}\DIFdelend \DIFaddbegin \DIFadd{L}\DIFaddend _c\DIFaddbegin \DIFadd{(t)}\DIFaddend +J_{ij}).
		\end{equation}
		\item When \DIFdelbegin \DIFdel{$X(t)=(s_i,l_0,l_c)$ and a time period of length $\tau$ passes without a jump occurring, the fluid level decreases by $r_i\tau$.
			If $l_c>0$, $L_c$ decreases but it cannot get lower than $0$ so it decreases by $\min\{l_c,r_i\tau\}$.
			When $l_c=0$, }\DIFdelend \DIFaddbegin \DIFadd{no jump occurs in a neighborhood of $t$, $S(t)$ remains constant and }\DIFaddend $L_0$ \DIFdelbegin \DIFdel{increases by $r_i\tau$.
			If $0<l_0<r_i\tau$ then first a time of $l_0/r_i$ passes so that $L_c(t+l_0/r_i)=0$ and then the remaining time passes so that $l_0$ increases by $r_i\tau-l_0$.This can be summarized in the following way
		}\begin{displaymath}\DIFdel{%DIFDELCMD < \label{eq:MmfmAgeEvolution}%%%
			(i,l_0,l_c)\stackrel\tau\mapsto (i,l_0+r_i\tau-\min\{l_c,r_i\tau\},l_c-\min\{l_c,r_i\tau\}).
		}\end{displaymath}
		%DIFAUXCMD
		\DIFdelend \DIFaddbegin \DIFadd{and $L_c$ evolve according to
		}\begin{equation}\DIFadd{\label{eq:MmfmAgeEvolution}
			\begin{split}
			\frac{d}{dt}L_c(t)&=\begin{cases}
			0&\ \text{if }L_c(t)=0,\\
			-r_{S(t)}&\ \text{else.}
		...
		\DIFadd{Note that we then always have
		}\[
		\DIFadd{\frac{d}{dt}\left[L_c(t)-L_0(t)\right]=\frac{d}{dt}Q(t)=-r_{S(t)}.
		}\]
		\item \DIFadd{When the asset breaks (i.e. whenever $Q(t)=0$ or equivalently $L_0(t^-)=Q_0$) the process moves to $x_{BREAK}$:
		}\[
		\DIFadd{X(t)=x_{BREAK}.
		}\]
		\DIFaddend \end{itemize}
	\DIFdelbegin \DIFdel{Then $Q(t)$, }\DIFdelend \DIFaddbegin \DIFadd{With these definitions, $Q(t)$ }\DIFaddend is again given by \eqref{eq:SimpleCurrentLevel} and it has distribution $F_{X(t)}(q)$,  again given by \eqref{eq:SimpleCurrentDistribution}.
	
	\begin{example}\label{example:MmfmState}
		Consider the MMFM depicted by figure \ref{figure:MmfmExample}.
		Now consider the following run of the \DIFdelbegin \DIFdel{machine}\DIFdelend \DIFaddbegin \DIFadd{asset}\DIFaddend :
		\begin{itemize}
			\item The \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend starts in $s_1$ with initial fluid level $5$
			\item After 1 time unit, a transition to $s_2$ occurs
			\item The \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend stays in $s_1$ for half a time unit, then it transitions to $s_3$
		...
		As you can see, $L_0(t)$ is nondecreasing and the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend breaks when $L_0(t)=Q(0)$.
	\end{example}
	\begin{figure}
		\DIFdelbeginFL %DIFDELCMD < \label{figure:MmfmExample}
		%DIFDELCMD < %%%
		\DIFdelendFL \centering
		\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
		thick,main node/.style={circle,draw,font=\sffamily\Large\bfseries}]
		...
		\DIFaddbeginFL \label{figure:MmfmExample}
		\DIFaddendFL \end{figure}
	\begin{figure}[H]
		\DIFdelbeginFL %DIFDELCMD < \label{figure:MmfmExampleQuantities}
		%DIFDELCMD < %%%
		\DIFdelendFL \centering
		\setlength\fwidth{0.7\textwidth}
		\definecolor{mycolor1}{rgb}{0.00000,0.44700,0.74100}
		...
		\DIFaddbeginFL \label{figure:MmfmExampleQuantities}
		\DIFaddendFL \end{figure}
	
	Now, for \DIFaddbegin \DIFadd{defining the MDP, we will briefly return to discretized time.
		For }\DIFaddend $x_k=X(t_k)$, we describe the random \DIFdelbegin \DIFdel{disturbances }\DIFdelend \DIFaddbegin \DIFadd{variables }\DIFaddend $\omega_k=\omega_k(x_k)$ of the Markov decision process as the (index of the) state of the continuous Markov chain at the next decision stage or $\Omega_{BREAK}$ if the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend will break before then:
	\[
	\omega_k(x_k):=\DIFdelbegin %DIFDELCMD < \begin{cases}
	%DIFDELCMD < \Omega_{BREAK},&\ \text{if the machine breaks,}\\
	%DIFDELCMD < S(t_{k+1}),&\ \text{else.}
	%DIFDELCMD < \end{cases}
	%DIFDELCMD < %%%
	\DIFdelend \DIFaddbegin \begin{cases}
	...
	\subsection{Optimal policy and Bellman equations}
	We want to find a stationary policy \DIFdelbegin \DIFdel{$\mu:X\rightarrow \{a_W,a_R\}$ }\DIFdelend \DIFaddbegin \DIFadd{$\pi= \{\mu_1,...,\mu^N\}$ }\DIFaddend that chooses the action \DIFdelbegin \DIFdel{$u_k=\mu(x_k)$ }\DIFdelend \DIFaddbegin \DIFadd{$u_k=\pi(x_k)=\mu_i(i,l_0,l_c)$ }\DIFaddend that minimizes the expected total discounted cost \DIFdelbegin \DIFdel{$V_\delta(x_k,\mu)$ }\DIFdelend \DIFaddbegin \DIFadd{$V_\delta(x_k,\pi)$ }\DIFaddend for each state \DIFdelbegin \DIFdel{$x_k$}\DIFdelend \DIFaddbegin \DIFadd{$x_k=(i,l_0,l_c)$}\DIFaddend .
	Similarly as in the definition of age-based maintenance, \DIFdelbegin \DIFdel{$V_\delta(x_k,\mu)$ }\DIFdelend \DIFaddbegin \DIFadd{$V_\delta(x_k,\pi)$ }\DIFaddend is given by
	\[V_\delta(x_k,\DIFdelbegin \DIFdel{\mu}\DIFdelend \DIFaddbegin \DIFadd{\pi}\DIFaddend )=g(x_k,\DIFdelbegin \DIFdel{\mu}\DIFdelend \DIFaddbegin \DIFadd{\pi}\DIFaddend (x_k))+\alpha_\delta \DIFdelbegin \DIFdel{\mathbb{E}[V_\delta(S(x_k),\mu)]}\DIFdelend \DIFaddbegin \DIFadd{\mathbb{E}[V_\delta(S(x_k),\pi)]}\DIFaddend .\]
	The Bellman equations for the optimal cost \DIFdelbegin \DIFdel{$V_\delta(x_k,\mu^*)$ read
	}\DIFdelend \DIFaddbegin \DIFadd{$V_\delta(x_k,\pi^*)$ read
	}\DIFaddend \begin{equation}\label{eq:MmfmBellman}
	V_\delta(x_k,\DIFdelbegin \DIFdel{\mu}\DIFdelend \DIFaddbegin \DIFadd{\pi}\DIFaddend ^*)=\DIFdelbegin %DIFDELCMD < \begin{cases}
	%DIFDELCMD < c+a+\alpha_\delta V_\delta(x_{NEW},\mu^*),&\text{ if }x_k=x_{BREAK},\\
	%DIFDELCMD < \min\left\{\begin{split}&c+\alpha_\delta V_\delta(x_{NEW},\mu^*),\\&\alpha_\delta \mathbb{E}[V_\delta(S(x_k),\mu^*)]\end{split}\right\},&\text{else.}\\
	%DIFDELCMD < \end{cases}
	%DIFDELCMD < %%%
	\DIFdelend \DIFaddbegin \begin{cases}
	...
	\end{cases}
	\DIFaddend \end{equation}
	\DIFdelbegin \DIFdel{$\mu$ }\DIFdelend \DIFaddbegin \DIFadd{$\pi$ }\DIFaddend is optimal if \DIFdelbegin \DIFdel{$V_\delta(x,\mu)=V_\delta(x,\mu^*)$ for all $x$.
		$\mathbb{E}[V_\delta(S(x_k),\mu^*)]$ }\DIFdelend \DIFaddbegin \DIFadd{$V_\delta(x,\pi)=V_\delta(x,\pi^*)$ for all $x\in X$.
		$\mathbb{E}[V_\delta(S(x_k),\pi^*)]$ }\DIFaddend when $x_k\neq x_{BREAK}$ is given by 
	\begin{equation}\label{eq:MmfmNextState}
	\DIFdelbegin %DIFDELCMD < \begin{split}
	%DIFDELCMD < &\mathbb{E}[V_\delta(S(i,l_0,l_c),\mu^*)]\\
	%DIFDELCMD < &=\begin{cases}
	%DIFDELCMD < \begin{split}
	%DIFDELCMD < &\sum\limits_{j\neq i}(1-e^{-\lambda_i \delta})V_\delta(j,l_0,l_c+J_{ij}-r_j\delta,\mu^*)\\
	%DIFDELCMD < &+e^{-\lambda_i \delta}V_\delta(i,l_0,l_c-r_i\delta,\mu^*),
	%DIFDELCMD < \end{split}%%%
	\DIFdelend \DIFaddbegin \begin{split}
	...
	\end{split}\DIFaddend &\ \text{If $l_c>0$,}\\
	\DIFdelbegin %DIFDELCMD < \begin{split}
	%DIFDELCMD < &e^{-\lambda_i \delta} \bar{F}_{t_k}(l_0+r_i\delta)V_\delta(i,l_0+r_i\delta,0,\mu^*)\\
	%DIFDELCMD < &+ e^{-\lambda_i \delta}F_{t_k}(l_0+r_i\delta)V_\delta(x_{BREAK},\mu^*)\\
	...
	%DIFDELCMD < &+(1-\lambda_i \delta)V_\delta(i,l_0,l_c-r_i\delta,\mu^*)+o(\delta^2),
	%DIFDELCMD < \end{split}
	%DIFDELCMD < &\ \text{If $l_c>0$,}\\
	%DIFDELCMD < \begin{split}
	%DIFDELCMD < &(1-\lambda_i \delta-\delta r_ih(l_0))V_\delta(i,l_0+r_i\delta,0,\mu^*)\\
	%DIFDELCMD < &+ \delta r_ih(l_0)V_\delta(x_{BREAK},\mu^*)\\
	%DIFDELCMD < &+\sum\limits_{j\neq i}\lambda_{ij} \delta V_\delta(j,l_0,J_{ij}-r_j\delta,\mu^*)+o(\delta^2),
	%DIFDELCMD < \end{split}&\ \text{If $l_c=0$.}\\
	%DIFDELCMD < \end{cases}
	%DIFDELCMD < %%%
	\DIFdelend \DIFaddbegin \begin{cases}
	\begin{split}
	...
	\DIFaddbegin \subsection{\DIFadd{Continuous-time MDP}}
	\DIFadd{Again, we will consider a continuous-time MDP by letting $\delta\rightarrow0$.
	}
	
	\DIFaddend \begin{remark}
		Note that the simple fluid model \DIFaddbegin \DIFadd{of the previous chapter }\DIFaddend corresponds to a MMFM model with two states, both with fluid rate $1$, transition rate $\lambda$ and jump size $J$.
	...
	Again, we could model the problem so that the choice to repair the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend can only be made at the instant that a transition occurs.
	This might be more realistic for similar reasons as for the simple fluid model: The jump could be caused by some mechanic performing some partial maintenance and a mechanic might be needed to completely repair the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend so that CTMC-transitions are the only opportunities to repair the \DIFdelbegin \DIFdel{machine}\DIFdelend \DIFaddbegin \DIFadd{asset}\DIFaddend .
	...
	\subsection{Stationary policy}
	By the same reasoning as for the previous \DIFdelbegin \DIFdel{problems}\DIFdelend \DIFaddbegin \DIFadd{chapter}\DIFaddend , we can prove that \DIFdelbegin \DIFdel{in each state $x\in X$, the optimal choice depends only on the distribution $F_x$ of the remaining fluid, which only depends on the state $x$ of the process}\DIFdelend \DIFaddbegin \DIFadd{every discrete approximation of the continuous-time MDP has a countable state space so that section 6.2.4 of \mbox{%DIFAUXCMD
			\cite{Puterman2008} }%DIFAUXCMD
		again proves that the there exists a stationary policy that is discount-optimal}\DIFaddend .
	
	\subsection{Empty buffer}
	By the same reasoning as for the simple fluid problem, an optimal policy will never perform preventive maintenance if the buffer \DIFaddbegin \DIFadd{$L_c$ }\DIFaddend is non-empty.
	...
	\]
	Note that these expectations do not depend on calendar time but only on the \DIFdelbegin \DIFdel{CTMC-state the process is in when it }\DIFdelend \DIFaddbegin \DIFadd{state of the background CTMC when the fluid }\DIFaddend is at level $Q(t)$.
	...
	\]
	Where these are similar to the earlier defined $D_{ij}$ but without the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend being repaired before fluid level $Q(t)-q$ is reached.
	...
	\DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{Preventive maintenance }\DIFaddend has a cost $c$, discounted at $D_{i}(\mu_i,\pi,0)$.
	...
	In this case, repair is \DIFdelbegin \DIFdel{at }\DIFdelend \DIFaddbegin \DIFadd{chosen }\DIFaddend in some CTMC-state $s_i$ and $l_0>\mu_i$.
	...
	\begin{theorem}
		...
		\DIFdelbegin %DIFDELCMD < \begin{split}
		%DIFDELCMD < 	V(x_{NEW},\pi)=&\mathbb{E}[D_{i}(Q_0,\pi,0)](c+a+V(x_{NEW},\pi))\\
		%DIFDELCMD < 	&+\left[\int\limits_0^\infty \bar{F}(q)\Gamma_0(q,\pi,0)dq+\sum\limits_i\bar{F}(\mu_i)D_{i}(\mu_i,\pi,0)\right](c+V(x_{NEW},\pi)).
		%DIFDELCMD < 	\end{split}
		%DIFDELCMD < 	%%%
		\DIFdelend \DIFaddbegin \begin{split}
	...
	\DIFaddbegin \begin{remark}
		...
		\DIFadd{D_i^t(q)=\mathbb{E}[e^{-\beta T_t(q)}|S(t)=i]
		}\]	
		\DIFadd{equals the value of the moment generating function of $T_t(q)$ (conditioned on $S(t)=i$) evaluated at $-\beta$.
			Furthermore, when all jump quantities are 0 (i.e. $J_{ij}=0$ for all $i,j$), $T_t(q)$ corresponds to the busy period of the fluid flow process considered in \mbox{%DIFAUXCMD
				\cite{Asmussen1994}}%DIFAUXCMD
			. 
			\mbox{%DIFAUXCMD
				\cite{Asmussen1994} }%DIFAUXCMD
			derives the moment generating function of this busy period, which also corresponds to our results.
	}\end{remark}
	
	\DIFaddend \subsection{Disregarding failures and policies}
	...
	\begin{lemma}\label{lemma:MmfmDiscountsInfinitisimal}
		...
		\begin{proof}
			...
			\[
			\DIFdelbegin %DIFDELCMD < \begin{split}
			%DIFDELCMD < 		D_{ij}(\delta r_i)&=(1-\delta\lambda_i)(1-\delta\beta)\mathds{1}\{i=j\}+\sum\limits_{k\neq i}\delta(1-\delta\beta)\lambda_{ik}D_{kj}(J_{ik})+o(\delta^2)\\
			%DIFDELCMD < 		&(1-\delta\lambda_i-\delta\beta)\mathds{1}\{i=j\}+\sum\limits_{k\neq i}\delta\lambda_{ik}D_{kj}(J_{ik})+o(\delta^2).
			%DIFDELCMD < 		\end{split}
			%DIFDELCMD < 		%%%
			\DIFdelend \DIFaddbegin \begin{split}
	...
	\begin{lemma}
		...
		\begin{proof}
			...
			\[
			\DIFdelbegin %DIFDELCMD < \begin{split}
			%DIFDELCMD < 		D_{ij}(q+\delta r_i)&=\sum_mD_{im}(\delta r_i)D_{mj}(q)\\
			%DIFDELCMD < 		&=\sum\limits_m\left[(1-\delta\lambda_i-\delta\beta)\mathds{1}\{i=j\}+\sum\limits_{k\neq i}\delta\lambda_{ik}D_{kj}(J_{ik})+o(\delta^2)\right]D_{mj}(q)\\
			%DIFDELCMD < 		&=(1-\delta\lambda_i-\delta\beta)D_{ij}(q)+\sum\limits_m\left[\sum\limits_{k\neq i}\delta\lambda_{ij}D_{kj}(J_{ik})\right]D_{mj}(q)+o(\delta^2)\\
			%DIFDELCMD < 		&=D_{ij}(q)+\delta\left( \sum\limits_m\left[\sum\limits_{k\neq i}\lambda_{ik}D_{km}(J_{ik})\right] D_{mj}(q)-(\lambda_i+\beta)D_{ij}(q) \right).\\
			%DIFDELCMD < 		\end{split}
			%DIFDELCMD < 		%%%
			\DIFdelend \DIFaddbegin \begin{split}
	...
	\begin{theorem}
		...
		D(q)=e^{\Lambda^D q}.
		\end{equation}
		So that the discounted probability \DIFdelbegin \DIFdel{from going to }\DIFdelend \DIFaddbegin \DIFadd{of going from }\DIFaddend state $i$ to $j$ while $Q(t)$ decreases by $q$, is given by
		\begin{equation}\DIFaddbegin \label{eq:MmfmDiscountEntrySolution}
		\DIFaddend D_{ij}(q)=\left(e^{\Lambda^D q}\right)_{ij}.
		\end{equation}
		\begin{proof}
			...
			\DIFaddbegin \DIFadd{This combined with $D(0)=I$ completes the proof.
			}\DIFaddend \end{proof}
	\end{theorem}
	...
	\begin{remark}\label{remark:MmfmStochasticShortestPath}
		As $L_0(t)$ increases continuously when $L_c(t)=0$ and is constant when $L_c(t)>0$, we know that in each run of the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend for each value $l_0\geq0$, there exists a $t$ so that $L_0(t)=l_0$.
		If we omit all time intervals where $L_c(t)>0$, we can view the \DIFdelbegin \DIFdel{machine }\DIFdelend \DIFaddbegin \DIFadd{asset }\DIFaddend as a CTMC over $L_0$.
		That is, \DIFdelbegin \DIFdel{using $L_0$ as time parameter.
			This adjusted CTMC would have generator $\Lambda^D$ so that the probability that the process is in CTMC-state $s_j$ at the time when $L_0(t)=l_0+q$, given that the process is in CTMC-state }\DIFdelend \DIFaddbegin \DIFadd{for the first run of the asset, we consider a CTMC $S^*(l_0)$ where
		}\[
		\DIFadd{S^*(l_0):=S(\min\{\tau|L_c(\tau)=0\wedge L_0(\tau)=l_0\}).
		}\]
		\DIFadd{When we would start in a state }\DIFaddend $s_i$ \DIFdelbegin \DIFdel{when $L_0(t)=l_0$, is given by }\[
		\DIFdel{\left(e^{\Lambda^Dq}\right)_{ij}.
		}\]
		%DIFAUXCMD
		\DIFdelend \DIFaddbegin \DIFadd{and pay a certain cost when $X(t)=(i,l_0,0)$, the expected value at which this cost would be discounted, would correspond to }\eqref{eq:MmfmDiscountEntrySolution}\DIFadd{.
			Transition probabilities of $S^*$ are given by generator $e^{\Lambda^D}$ for $\beta=0$.
		}\DIFaddend Viewing the process as this adjusted CTMC \DIFdelbegin \DIFdel{can simplify }\DIFdelend \DIFaddbegin \DIFadd{helps understanding }\DIFaddend the problem.
	\end{remark}
	...
	\DIFdelbegin %DIFDELCMD < \begin{remark}
	%DIFDELCMD < 	%%%
	\DIFdel{Referring back to remark \ref{remark:MmfmStochasticShortestPath}, if we view the process as this adjusted CTMC, the problem simplifies to a stochastic shortest path problem where in each state $s_i$, there is a transition to a terminating state with rate
	}\[
	\DIFdel{-\sum\limits_j \Lambda^D_{ij},
	}\]
	%DIFAUXCMD
	\DIFdel{and a terminating cost of 0.
	}%DIFDELCMD < \end{remark} %%%
	\DIFdelend %DIF > 
	\section{The optimal policy}
	In this section, we will analytically derive the control limits for the \DIFdelbegin \DIFdel{MMFM preventive }\DIFdelend \DIFaddbegin \DIFadd{optimal MMFM }\DIFaddend maintenance policy.
	...
	\[
	\sum\limits\DIFdelbegin \DIFdel{_j\Lambda^D}\DIFdelend \DIFaddbegin \DIFadd{_{j\neq i}\lambda}\DIFaddend _{ij}V(j,\mu_i^*,\DIFdelbegin \DIFdel{0}\DIFdelend \DIFaddbegin \DIFadd{J_{ij}}\DIFaddend ,\pi^*).
	\]
	This term is caused by the possibility that a jump would occur \DIFaddbegin \DIFadd{around the time the control limit is reached}\DIFaddend .
	If we would simply assume that no jump would occur \DIFaddbegin \DIFadd{before the next failure}\DIFaddend , we could omit this term and the problem would be easier to solve.
	...
	\begin{figure}[H]
		\DIFdelbeginFL %DIFDELCMD < \label{figure:distribution} 
		%DIFDELCMD < 	%%%
		\DIFdelendFL \setlength\fwidth{0.4\textwidth}
		\definecolor{mycolor1}{rgb}{0.00000,0.44700,0.74100}
		\begin{tikzpicture}
		...
		\DIFaddbeginFL \label{figure:distribution} 
		\DIFaddendFL \end{figure}
	...
	\begin{figure}[H]
		\DIFdelbeginFL %DIFDELCMD < \label{figure:hazard}
		%DIFDELCMD < 	%%%
		\DIFdelendFL \centering
		...
		\DIFaddbeginFL \label{figure:hazard}
		\DIFaddendFL \end{figure}
	...
	\DIFdelbegin %DIFDELCMD < [%%%
	\DIFdel{Summarize results}%DIFDELCMD < ]
	%DIFDELCMD < %%%
	\DIFdelend \DIFaddbegin 
	
	\DIFadd{In this thesis, discounted preventive maintenance problems were considered.
	...
	\DIFadd{We showed that for the age-based preventive maintenance problem, the optimal policy is a stationary control limit policy where maintenance is chosen whenever the age reaches the optimal control limit $\mu^*$.
		If $\mu^*<\infty$, then this $\mu^*$ is the solution to the equation
	}\begin{equation}\DIFadd{\label{eq:SummaryAgeControlLimit}
		h(\mu^*)=\beta\frac{c+V(0^+,\mu^*)}{a},
	}\end{equation}
	\DIFadd{where $V(0^+,\mu^*)$ denotes the expected total discounted cost.
		A policy iteration method was introduced and its convergence was proven.
	}
	
	\DIFadd{For the simple fluid problem, we proved that the optimal policy is a stationary control limit policy where maintenance is chosen when the amount of used initial fluid exceeds a certain threshold $\mu^*$.
	...
	\DIFadd{\beta^*=\beta+\lambda(1-D(J)),
	}\]
	\DIFadd{where $D(J)$ denotes the expected value of the discount factor over the period in which the fluid level decreases by $J$.
	...
	\DIFadd{We extended this simple fluid problem to a problem where the degradation of the asset is modeled by a MMFM with jumps.
	...
	\DIFadd{Also, a method to estimate the fluid rates and the jump quantities was presented that assumes the distribution of the initial fluid level has a small variance.
	}
	
	\DIFaddend \section{Further research}
	\DIFdelbegin %DIFDELCMD < [%%%
	\DIFdel{Random jump sizes, better parameter estimation methods, zero or negative fluid rates , nonincreasing hazard rates, ordering the CTMC states depending only on the MMFM}%DIFDELCMD < ]
	%DIFDELCMD < %%%
	\DIFdelend \DIFaddbegin \DIFadd{We expect that the obtained results can be extended to models allowing states with zero fluid rates or fluid rates of opposite sign.
		The results might also be extended to initial distributions without monotonously increasing hazard rates.
	}\DIFaddend 
	
	\DIFaddbegin \DIFadd{The model that was considered in this thesis was simplified in order to make the analysis feasible.
		...
		Also, similar to the second order fluid model as considered in \mbox{%DIFAUXCMD
			\cite{Gribaudo2007}}%DIFAUXCMD
		, the fluid decrease could be modeled as Brownian motion.
	...
	\DIFadd{Better methods for estimating the fluid parameters (fluid rates and jump sizes) from trace data could also be researched.
	}
	
	\DIFaddend \chapter{Discussion}\label{chapter:discussion}
	\DIFaddbegin \DIFadd{We briefly discuss the assumptions on which this thesis relies and what happens when these assumptions fail.
		The effect of small changes in problem parameters is also discussed.
	}\DIFaddend \section{Assumptions}
	\DIFdelbegin %DIFDELCMD < [%%%
	\DIFdel{List assumptions , their consequences and alternatives}%DIFDELCMD < ]
	%DIFDELCMD < %%%
	\section{\DIFdel{Robustness}}
	%DIFAUXCMD
	\addtocounter{section}{-1}%DIFAUXCMD
	%DIFDELCMD < [%%%
	\DIFdel{Explain what happens to the resulting policy }\DIFdelend \DIFaddbegin \DIFadd{We assumed perfect repairs.
	...
	\DIFadd{We also assumed the cumulative distribution function of the initial fitness of the asset to be continuously differentiable.
	...
	\DIFadd{The hazard rate was also assumed to be increasing and continuous.
		Discontinuities in the hazard rate could result in the equations for the control limits (}\eqref{eq:AgeBasedHazardBound} \DIFaddend and \DIFaddbegin \eqref{eq:MmfmHazardBoundsShort}\DIFadd{) not having solutions.
	...
	\DIFadd{It was also assumed that the transitions in the Markovian environment occur independent of the condition (age or fluid level) of the asset.
		In practice, this assumption might fail as the degradation of the asset might be visible in its behavior.
		\mbox{%DIFAUXCMD
			\cite{Scheinhardt1998} }%DIFAUXCMD
		discusses models where the transitions in the Markovian environment do depend on the fluid level.
	}
	
	\DIFadd{The assumption of constant known fluid jump sizes might also be unrealistic.
		This assumption was mainly made to simplify the problem.
		The same holds for the assumption of exponential transition times.
	}
	
	\section{\DIFadd{Robustness}}
	\DIFadd{Small errors in the problem parameters could be caused by changes in the environment.
		For example, if the cost of maintenance would turn out to be larger than the cost $c$ that was used during the computation of the policy, this would result in the real }\DIFaddend total discounted cost \DIFdelbegin \DIFdel{when one of the problem parameters or MMFM parameters changes a little.
	}%DIFDELCMD < ]
	%DIFDELCMD < %%%
	\DIFdelend \DIFaddbegin \DIFadd{being larger than calculated.
		Furthermore, the chosen control limit would be too high. 
	}\DIFaddend 
	
	\DIFaddbegin \DIFadd{Most of the effects varying parameters are covered in sections \ref{section:AgeBasedStructuralProperties}, \ref{section:SimpleStructuralProperties} and \ref{section:MmfmStructuralProperties} for the age-based problem, the simple fluid problem and the MMFM problem respectively.
	}
	
	
	\DIFaddend \bibliography{Sections/bibliography}
...
%		%DIFDELCMD < 		%%%
%		\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The symbols and notation that are used in chapters \ref{chapter:AgeBased}, \ref{chapter:SimpleFluid} and \ref{chapter:Mmfm} are listed in the table below.
%		}\begin{tabularx}{\linewidth}{l|L}\label{table:Symbols}
%			\DIFaddendFL Symbol        & Meaning         \\
%			\DIFdelbeginFL \DIFdelFL{$V_\delta(x)$ }\DIFdelendFL \DIFaddbeginFL \hline                       
%			\DIFaddFL{$X$			}\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{Expected remaining total discounted cost when the machine has age $x\delta$                    }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{State space of the MDP. }\DIFaddendFL \\
%			\DIFdelbeginFL \DIFdelFL{$V(x,\mu)$    }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$X(t)$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{Expected remaining total discounted cost when the machine ahs age }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{State of the process at time $t$ }\\
%			\DIFaddFL{$\omega_k$ }& \DIFaddFL{Random variable for the evolution of the stochastic process at time $t_k$.}\\   
%			\DIFaddFL{$x_{BREAK}$ }& \DIFaddFL{State for when the asset is broken.           }\\
%			\DIFaddFL{$x_{NEW}$ }& \DIFaddFL{State for when the asset is new.}\\   
%			\DIFaddFL{$U(x)$}& \DIFaddFL{Set of available actions in state }\DIFaddendFL $x$\DIFdelbeginFL \DIFdelFL{, following policy }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{.}\\
%			\DIFaddFL{$a_W$}& \DIFaddFL{Action corresponding to doing nothing.}\\
%			\DIFaddFL{$a_R$}& \DIFaddFL{Action corresponding to repairing (performing maintenance). }\\
%			\DIFaddFL{$g(x,u)$    }& \DIFaddFL{Cost of performing action $u$ in state $x\in X$.}\\
%			\DIFaddFL{$c$}& \DIFaddFL{Cost for maintenance.}\\
%			\DIFaddFL{$a$}& \DIFaddFL{Additional cost for corrective maintenance.}\\
%			\DIFaddFL{$\alpha_\delta$ }& \DIFaddFL{Discount over a period $\delta$. In this thesis, we use $\alpha_\delta=e^{-\beta \delta}$ for discount coefficient $\beta>0$.}\\       
%			\DIFaddendFL $\mu$ \DIFaddbeginFL & \DIFaddFL{Control limit. $\mu^*$ denotes the optimal control limit.}\\
%			\DIFaddFL{$\pi$}& \DIFaddFL{The policy for the MMFM-problem}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{$\pi=\{\mu_1,...,\mu_N\}$ where $\mu_i$ denotes the control limit in CTMC-state $s_i$. $\pi^*$ denotes the optimal policy.}\DIFaddendFL \\
%			$x\wedge y$   & The minimum of $x$ and $y$.          \\       
%			$x\vee y$     & The maximum of $x$ and $y$.\DIFdelbeginFL %DIFDELCMD < \end{tabular}
%			%DIFDELCMD < \end{table} 	%%%
%			\DIFdelend \DIFaddbegin \\
%			\DIFadd{$Q_0$ }& \DIFadd{Distribution of the initial fitness (lifetime or fluid level) of the asset.}\\
%			\DIFadd{$Q(t)$}& \DIFadd{Fluid level at time $t$.}\\
%			\DIFadd{$L_0(t)$}& \DIFadd{Amount of used initial fluid at time $t$.}\\
%			\DIFadd{$L_c(t)$}& \DIFadd{Amount of fluid inside the buffer at time $t$.}\\
%			\DIFadd{$F$ }& \DIFadd{CDF of $Q_0$.}\\
%			\DIFadd{$F_{X(t)}$ }& \DIFadd{CDF of $Q(t)$}\\
%			\DIFadd{$\bar F(x)$}& \DIFadd{Reliability function corresponding to $F$. Furthermore $\bar F(x;y)=\frac{\bar F(x)}{\bar F(y)}$.}\\
%			\DIFadd{$h$}& \DIFadd{Hazard rate of $Q_0$.}\\                       
%			\DIFadd{$V_\delta(x,\mu)$ }& \DIFadd{Expected total discounted cost at state $x$ with time-step $\delta$, using control limit $\mu$. $\delta$ is omitted than for the continuous MDP. For the MMFM we write $V_\delta(x,\pi)$ with policy $\pi$.            }\\
%			\DIFadd{$\lambda_{ij}$}& \DIFadd{Transition rate from state $s_i$ to $s_j$. Also $\lambda_i=-\sum_{j\neq i}\lambda_{ij}$. $\lambda$ is used for the simple fluid model.}\\
%			\DIFadd{$J_{ij}$}& \DIFadd{Jump quantity when transitioning from $s_i$ to $s_j$. $J$ for the simple fluid model.}\\
%			\DIFadd{$N$}& \DIFadd{Number of CTMC-states for the MMFM problem.}\\
%			\DIFadd{$T_t(q)$}&\DIFadd{Random variable denoting the time until the fluid level has decreased by $q$, i.e. $\min\{\tau-t>0|Q(\tau)=Q(t)-q\}$.}\\
%			\DIFadd{$N_t(q)$}&\DIFadd{The number of jumps that occur during the time that the fluid decreases by $q$.}\\
%			\DIFadd{$D_{ij}^t(q,\pi,l)$}&\DIFadd{Expected value of the discount factor over the time period that the process reaches state $s_j$ while the fluid level decreases by $q$ without a control limit from $\pi$ being encountered, given that at time $t$ the process was in $X(t)= (i,l,0)$. For the simple fluid problem, $D(q)$ denotes the discount over the time that the fluid decreases by $q$.}\\
%			\DIFadd{$\Lambda^D$}&\DIFadd{The generator matrix for $D_{ij}$.}\\
%		\end{tabularx}
		%DIF > 
		...
		\begin{corollary}
			If the hazard rate of the lifetime \DIFdelbegin \DIFdel{$Q$ of the machine }\DIFdelend \DIFaddbegin \DIFadd{$Q_0$ of the asset }\DIFaddend is monotonously decreasing, preventive repair will never be the optimal choice.
		\end{corollary} \chapter{\DIFdelbegin \DIFdel{CTMC analysis}\DIFdelend \DIFaddbegin \DIFadd{Total discounted costs for various problem parameters and policies?}\DIFaddend }\DIFaddbegin \label{AppendixComputationsTable}
		\DIFaddend \section{\DIFdelbegin \DIFdel{Value iteration}\DIFdelend \DIFaddbegin \DIFadd{Age-based problem}\DIFaddend }
		\DIFdelbegin %DIFDELCMD < [%%%
		\DIFdel{Derive Bellman equations, mention implementation}%DIFDELCMD < ]
		%DIFDELCMD < %%%
		\section{\DIFdel{State clustering}}
		%DIFAUXCMD
		\addtocounter{section}{-1}%DIFAUXCMD
		%DIFDELCMD < [%%%
		\DIFdel{Explain the state clustering that was employed, mention that repair states tend to be clustered and explain}%DIFDELCMD < ]
		%DIFDELCMD < %%%
		\chapter{\DIFdel{Total discounted costs for various problem parameters and policies?}}%DIFAUXCMD
		\addtocounter{chapter}{-1}%DIFAUXCMD
		%DIFDELCMD < \label{AppendixComputationsTable}
		%DIFDELCMD < [%%%
		\DIFdel{Tables containing the total discounted cost and control limit }\DIFdelend \DIFaddbegin \DIFadd{Below optimal control limits and corresponding total discounted costs of the age-based maintenance problem are listed for a few parameters.
		...
		\begin{tabularx}{\linewidth}{llllll}\label{table:AgeComputations}
			\DIFadd{$c$        }& \DIFadd{$a$ }& \DIFadd{$\beta$ }& \DIFadd{Lifetime }& \DIFadd{$\mu^*$ }& \DIFadd{$V(0^+,\mu^*)$        }\\
			\hline                       
			\DIFadd{$1.000$ }& \DIFadd{$100.000$ }& \DIFadd{$1.000$ }& \DIFadd{Weibull$(20/\pi,2)$ }& \DIFadd{$1.388$ }& \DIFadd{$1.181$ }\\ 
			\DIFadd{$1.000$ }& \DIFadd{$100.000$ }& \DIFadd{$1.000$ }& \DIFadd{Gamma$(2,5)$ }& \DIFadd{$2.349$ }& \DIFadd{$0.233$ }\\ 
			\DIFadd{$1.000$ }& \DIFadd{$5.000$ }& \DIFadd{$1.000$ }& \DIFadd{Weibull$(20/\pi,2)$ }& \DIFadd{$13.897$ }& \DIFadd{$0.091$ }\\ 
			\DIFadd{$1.000$ }& \DIFadd{$5.000$ }& \DIFadd{$1.000$ }& \DIFadd{Gamma$(2,5)$ }& \DIFadd{$10.298$ }& \DIFadd{$0.025$ }\\ 
			\DIFadd{$1.000$ }& \DIFadd{$100.000$ }& \DIFadd{$0.500$ }& \DIFadd{Weibull$(20/\pi,2)$ }& \DIFadd{$1.246$ }& \DIFadd{$2.916$ }\\ 
			\DIFadd{$1.000$ }& \DIFadd{$100.000$ }& \DIFadd{$0.500$ }& \DIFadd{Gamma$(2,5)$ }& \DIFadd{$2.123$ }& \DIFadd{$0.838$ }\\ 
			\DIFadd{$1.000$ }& \DIFadd{$100.000$ }& \DIFadd{$2.000$ }& \DIFadd{Weibull$(20/\pi,2)$ }& \DIFadd{$1.760$ }& \DIFadd{$0.383$ }\\ 
			\DIFadd{$1.000$ }& \DIFadd{$100.000$ }& \DIFadd{$2.000$ }& \DIFadd{Gamma$(2,5)$ }& \DIFadd{$2.831$ }& \DIFadd{$0.030$ }\\ 
		\end{tabularx}
		
		\section{\DIFadd{Simple fluid problem}}
		\DIFadd{Below optimal control limits and corresponding total discounted costs of the simple fluid model maintenance problem are listed for a few parameters.
			These control limits and total discounted costs were obtained using the iteration method presented in this thesis.
		}\begin{tabularx}{\linewidth}{llllllll}\label{table:SimpleFluidComputations}
			\DIFadd{$c$        }& \DIFadd{$a$ }& \DIFadd{$\beta$ }& \DIFadd{Initial fluid }& \DIFadd{$\lambda$ }& \DIFadd{$J$ }& \DIFadd{$\mu^*$ }& \DIFadd{$V(0^+,\mu^*)$        }\\
			\hline    
			\DIFadd{$1$ }& \DIFadd{$100$ }& \DIFadd{$1$ }& \DIFadd{Weibull$(20/\pi,2)$ }& \DIFadd{$1.000$ }& \DIFadd{$0.500$ }& \DIFadd{$1.572$ }& \DIFadd{$0.608$ }\\ 
			\DIFadd{$1$ }& \DIFadd{$100$ }& \DIFadd{$1$ }& \DIFadd{Gamma$(2,5)$ }& \DIFadd{$1.000$ }& \DIFadd{$0.500$ }& \DIFadd{$2.607$ }& \DIFadd{$0.075$ }\\ 
			\DIFadd{$1$ }& \DIFadd{$5$ }& \DIFadd{$1$ }& \DIFadd{Weibull$(20/\pi,2)$ }& \DIFadd{$1.000$ }& \DIFadd{$0.500$ }& \DIFadd{$54.762$ }& \DIFadd{$0.039$ }\\ 
			\DIFadd{$1$ }& \DIFadd{$5$ }& \DIFadd{$1$ }& \DIFadd{Gamma$(2,5)$ }& \DIFadd{$1.000$ }& \DIFadd{$0.500$ }& \DIFadd{$\infty$ }& \DIFadd{$0.005$ }\\ 
			\DIFadd{$1$ }& \DIFadd{$100$ }& \DIFadd{$0.5$ }& \DIFadd{Weibull$(20/\pi,2)$ }& \DIFadd{$1.000$ }& \DIFadd{$0.500$ }& \DIFadd{$1.341$ }& \DIFadd{$1.495$ }\\ 
			\DIFadd{$1$ }& \DIFadd{$100$ }& \DIFadd{$0.5$ }& \DIFadd{Gamma$(2,5)$ }& \DIFadd{$1.000$ }& \DIFadd{$0.500$ }& \DIFadd{$2.276$ }& \DIFadd{$0.335$ }\\ 
			\DIFadd{$1$ }& \DIFadd{$100$ }& \DIFadd{$2.0$ }& \DIFadd{Weibull$(20/\pi,2)$ }& \DIFadd{$1.000$ }& \DIFadd{$0.500$ }& \DIFadd{$2.114$ }& \DIFadd{$0.209$ }\\ 
			\DIFadd{$1$ }& \DIFadd{$100$ }& \DIFadd{$2.0$ }& \DIFadd{Gamma$(2,5)$ }& \DIFadd{$1.000$ }& \DIFadd{$0.500$ }& \DIFadd{$3.176$ }& \DIFadd{$0.009$ }\\ 
			\DIFadd{$1$ }& \DIFadd{$100$ }& \DIFadd{$1.0$ }& \DIFadd{Weibull$(20/\pi,2)$ }& \DIFadd{$0.500$ }& \DIFadd{$1.000$ }& \DIFadd{$1.513$ }& \DIFadd{$0.730$ }\\ 
			\DIFadd{$1$ }& \DIFadd{$100$ }& \DIFadd{$1.0$ }& \DIFadd{Gamma$(2,5)$ }& \DIFadd{$0.500$ }& \DIFadd{$1.000$ }& \DIFadd{$2.528$ }& \DIFadd{$0.104$ }\\ 
			\DIFadd{$1$ }& \DIFadd{$5$ }& \DIFadd{$1.0$ }& \DIFadd{Weibull$(20/\pi,2)$ }& \DIFadd{$0.500$ }& \DIFadd{$1.000$ }& \DIFadd{$18.346$ }& \DIFadd{$0.049$ }\\ 
			\DIFadd{$1$ }& \DIFadd{$5$ }& \DIFadd{$1.0$ }& \DIFadd{Gamma$(2,5)$ }& \DIFadd{$0.500$ }& \DIFadd{$1.000$ }& \DIFadd{$\infty$ }& \DIFadd{$0.008$ }\\ 
			\DIFadd{$1$ }& \DIFadd{$100$ }& \DIFadd{$0.5$ }& \DIFadd{Weibull$(20/\pi,2)$ }& \DIFadd{$0.500$ }& \DIFadd{$1.000$ }& \DIFadd{$1.319$ }& \DIFadd{$1.698$ }\\ 
			\DIFadd{$1$ }& \DIFadd{$100$ }& \DIFadd{$0.5$ }& \DIFadd{Gamma$(2,5)$ }& \DIFadd{$0.500$ }& \DIFadd{$1.000$ }& \DIFadd{$2.241$ }& \DIFadd{$0.403$ }\\ 
			\DIFadd{$1$ }& \DIFadd{$100$ }& \DIFadd{$2.0$ }& \DIFadd{Weibull$(20/\pi,2)$ }& \DIFadd{$0.500$ }& \DIFadd{$1.000$ }& \DIFadd{$1.970$ }& \DIFadd{$0.259$ }\\ 
			\DIFadd{$1$ }& \DIFadd{$100$ }& \DIFadd{$2.0$ }& \DIFadd{Gamma$(2,5)$ }& \DIFadd{$0.500$ }& \DIFadd{$1.000$ }& \DIFadd{$3.045$ }& \DIFadd{$0.014$ }\\ 
		\end{tabularx}
		\section{\DIFadd{Markov Modulated fluid model}}
		\DIFadd{For the following two MMFMs we calculated the total discounted costs of the preventive maintenance problem.
			...
			We denote the uniform control limit by $\mu_u$ and the control limits }\DIFaddend that resulted from \DIFdelbegin \DIFdel{various heuristic policies and parameters.
		}%DIFDELCMD < ]
		%DIFDELCMD < %%%
		\DIFdelend \DIFaddbegin \DIFadd{assuming no jumps will occur before the next failure by $\mu_i'$, with corresponding total discounted costs $V_u(\mu_u)$ and $V(0^+,\pi')$ respectively.
		}
		...
		\begin{landscape}
			...
			\begin{figure}
				...
				(3) edge node {$1:5$} (1);
				\end{tikzpicture}
				\caption{\DIFaddFL{Depiction of the MMFM corresponding to table \ref{table:MmfmComputations1}.}}
			...
			\begin{table}[H]
				...
				\begin{tabularx}{\linewidth}{llllllllllllll}
					\DIFaddFL{$c$}& \DIFaddFL{$a$ }& \DIFaddFL{$\beta$ }& \DIFaddFL{Initial fluid }& \DIFaddFL{$\mu_1^*$ }& \DIFaddFL{$\mu_2^*$ }& \DIFaddFL{$\mu_3^*$ }& \DIFaddFL{$V(0^+,\pi^*)$ }& \DIFaddFL{$\mu_u$ }& \DIFaddFL{$V_u(\mu_u)$ }&\DIFaddFL{$\mu_1'$ }& \DIFaddFL{$\mu_2'$ }& \DIFaddFL{$\mu_3'$ }&\DIFaddFL{$V(0^+,\pi')$  }\\
					\hline    
					\DIFaddFL{$1$ }& \DIFaddFL{$100$ }& \DIFaddFL{$1.0$ }& \DIFaddFL{Weibull$(20/\pi,2)$ }& \DIFaddFL{$1.839$ }& \DIFaddFL{$0.578$ }& \DIFaddFL{$1.144$ }& \DIFaddFL{$1.889$ }& \DIFaddFL{$1.749$ }& \DIFaddFL{$1.805$ }& \DIFaddFL{$1.657$ }& \DIFaddFL{$0.829$ }& \DIFaddFL{$0.552$ }& \DIFaddFL{$1.603$ }\\ 
					\DIFaddFL{$1$ }& \DIFaddFL{$100$ }& \DIFaddFL{$1.0$ }& \DIFaddFL{Gamma$(2,5)$ }& \DIFaddFL{$2.920$ }& \DIFaddFL{$1.964$ }& \DIFaddFL{$2.451$ }& \DIFaddFL{$1.235$ }& \DIFaddFL{$2.514$ }& \DIFaddFL{$0.494$ }& \DIFaddFL{$2.586$ }& \DIFaddFL{$2.032$ }& \DIFaddFL{$1.780$ }& \DIFaddFL{$0.614$ }\\ 
					\DIFaddFL{$1$ }& \DIFaddFL{$5$ }& \DIFaddFL{$1.0$ }& \DIFaddFL{Weibull$(20/\pi,2)$ }& \DIFaddFL{$19.966$ }& \DIFaddFL{$5.928$ }& \DIFaddFL{$12.424$ }& \DIFaddFL{$0.784$ }& \DIFaddFL{$15.226$ }& \DIFaddFL{$0.196$ }& \DIFaddFL{$18.585$ }& \DIFaddFL{$9.292$ }& \DIFaddFL{$6.195$ }& \DIFaddFL{$0.460$ }\\ 
					\DIFaddFL{$1$ }& \DIFaddFL{$5$ }& \DIFaddFL{$1.0$ }& \DIFaddFL{Gamma$(2,5)$ }& \DIFaddFL{$22.894$ }& \DIFaddFL{$6.948$ }& \DIFaddFL{$10.746$ }& \DIFaddFL{$0.719$ }& \DIFaddFL{$11.069$ }& \DIFaddFL{$0.096$ }& \DIFaddFL{$14.748$ }& \DIFaddFL{$7.246$ }& \DIFaddFL{$5.570$ }& \DIFaddFL{$0.365$ }\\ 
					\DIFaddFL{$1$ }& \DIFaddFL{$100$ }& \DIFaddFL{$0.5$ }& \DIFaddFL{Weibull$(20/\pi,2)$ }& \DIFaddFL{$1.559$ }& \DIFaddFL{$0.167$ }& \DIFaddFL{$1.207$ }& \DIFaddFL{$3.899$ }& \DIFaddFL{$1.332$ }& \DIFaddFL{$4.028$ }& \DIFaddFL{$1.485$ }& \DIFaddFL{$0.743$ }& \DIFaddFL{$0.495$ }& \DIFaddFL{$3.667$ }\\ 
					\DIFaddFL{$1$ }& \DIFaddFL{$100$ }& \DIFaddFL{$0.5$ }& \DIFaddFL{Gamma$(2,5)$ }& \DIFaddFL{$2.616$ }& \DIFaddFL{$1.365$ }& \DIFaddFL{$2.386$ }& \DIFaddFL{$2.334$ }& \DIFaddFL{$2.284$ }& \DIFaddFL{$1.316$ }& \DIFaddFL{$2.380$ }& \DIFaddFL{$1.883$ }& \DIFaddFL{$1.654$ }& \DIFaddFL{$1.560$ }\\ 
					\DIFaddFL{$1$ }& \DIFaddFL{$100$ }& \DIFaddFL{$2.0$ }& \DIFaddFL{Weibull$(20/\pi,2)$ }& \DIFaddFL{$2.091$ }& \DIFaddFL{$1.046$ }& \DIFaddFL{$1.036$ }& \DIFaddFL{$0.642$ }& \DIFaddFL{$2.142$ }& \DIFaddFL{$0.683$ }& \DIFaddFL{$2.061$ }& \DIFaddFL{$1.030$ }& \DIFaddFL{$0.687$ }& \DIFaddFL{$0.619$ }\\ 
					\DIFaddFL{$1$ }& \DIFaddFL{$100$ }& \DIFaddFL{$2.0$ }& \DIFaddFL{Gamma$(2,5)$ }& \DIFaddFL{$3.010$ }& \DIFaddFL{$2.333$ }& \DIFaddFL{$2.326$ }& \DIFaddFL{$0.210$ }& \DIFaddFL{$2.929$ }& \DIFaddFL{$0.126$ }& \DIFaddFL{$3.000$ }& \DIFaddFL{$2.326$ }& \DIFaddFL{$2.026$ }& \DIFaddFL{$0.199$ }\\ 
					\DIFaddFL{$1$ }& \DIFaddFL{$100$ }& \DIFaddFL{$1.0$ }& \DIFaddFL{Weibull$(20/\pi,2)$ }& \DIFaddFL{$2.167$ }& \DIFaddFL{$1.249$ }& \DIFaddFL{$0.467$ }& \DIFaddFL{$0.463$ }& \DIFaddFL{$2.477$ }& \DIFaddFL{$0.674$ }& \DIFaddFL{$0.962$ }& \DIFaddFL{$0.481$ }& \DIFaddFL{$0.321$ }& \DIFaddFL{$0.511$ }\\ 
					\DIFaddFL{$1$ }& \DIFaddFL{$100$ }& \DIFaddFL{$1.0$ }& \DIFaddFL{Gamma$(2,5)$ }& \DIFaddFL{$3.198$ }& \DIFaddFL{$2.590$ }& \DIFaddFL{$1.851$ }& \DIFaddFL{$0.211$ }& \DIFaddFL{$3.147$ }& \DIFaddFL{$0.164$ }& \DIFaddFL{$2.329$ }& \DIFaddFL{$1.846$ }& \DIFaddFL{$1.622$ }& \DIFaddFL{$0.204$ }\\ 
					\DIFaddFL{$1$ }& \DIFaddFL{$5$ }& \DIFaddFL{$1.0$ }& \DIFaddFL{Weibull$(20/\pi,2)$ }& \DIFaddFL{$0.000$ }& \DIFaddFL{$0.000$ }& \DIFaddFL{$0.000$ }& \DIFaddFL{$19.000$ }& \DIFaddFL{$31.477$ }& \DIFaddFL{$0.063$ }& \DIFaddFL{$15.266$ }& \DIFaddFL{$7.633$ }& \DIFaddFL{$5.089$ }& \DIFaddFL{$0.199$ }\\ 
					\DIFaddFL{$1$ }& \DIFaddFL{$5$ }& \DIFaddFL{$1.0$ }& \DIFaddFL{Gamma$(2,5)$ }& \DIFaddFL{$0.000$ }& \DIFaddFL{$0.000$ }& \DIFaddFL{$0.000$ }& \DIFaddFL{$19.000$ }& \DIFaddFL{$0.000$ }& \DIFaddFL{$19.000$ }& \DIFaddFL{$11.937$ }& \DIFaddFL{$6.503$ }& \DIFaddFL{$5.105$ }& \DIFaddFL{$0.169$ }\\ 
					\DIFaddFL{$1$ }& \DIFaddFL{$100$ }& \DIFaddFL{$0.5$ }& \DIFaddFL{Weibull$(20/\pi,2)$ }& \DIFaddFL{$1.881$ }& \DIFaddFL{$1.141$ }& \DIFaddFL{$0.362$ }& \DIFaddFL{$0.924$ }& \DIFaddFL{$2.514$ }& \DIFaddFL{$1.677$ }& \DIFaddFL{$0.688$ }& \DIFaddFL{$0.344$ }& \DIFaddFL{$0.229$ }& \DIFaddFL{$1.161$ }\\ 
					\DIFaddFL{$1$ }& \DIFaddFL{$100$ }& \DIFaddFL{$0.5$ }& \DIFaddFL{Gamma$(2,5)$ }& \DIFaddFL{$2.929$ }& \DIFaddFL{$2.433$ }& \DIFaddFL{$1.667$ }& \DIFaddFL{$0.469$ }& \DIFaddFL{$2.936$ }& \DIFaddFL{$0.478$ }& \DIFaddFL{$1.967$ }& \DIFaddFL{$1.576$ }& \DIFaddFL{$1.392$ }& \DIFaddFL{$0.462$ }\\ 
					\DIFaddFL{$1$ }& \DIFaddFL{$100$ }& \DIFaddFL{$2.0$ }& \DIFaddFL{Weibull$(20/\pi,2)$ }& \DIFaddFL{$2.760$ }& \DIFaddFL{$1.491$ }& \DIFaddFL{$0.665$ }& \DIFaddFL{$0.201$ }& \DIFaddFL{$2.821$ }& \DIFaddFL{$0.228$ }& \DIFaddFL{$1.527$ }& \DIFaddFL{$0.764$ }& \DIFaddFL{$0.509$ }& \DIFaddFL{$0.199$ }\\ 
					\DIFaddFL{$1$ }& \DIFaddFL{$100$ }& \DIFaddFL{$2.0$ }& \DIFaddFL{Gamma$(2,5)$ }& \DIFaddFL{$3.654$ }& \DIFaddFL{$2.850$ }& \DIFaddFL{$2.135$ }& \DIFaddFL{$0.076$ }& \DIFaddFL{$3.589$ }& \DIFaddFL{$0.032$ }& \DIFaddFL{$2.876$ }& \DIFaddFL{$2.239$ }& \DIFaddFL{$1.953$ }& \DIFaddFL{$0.074$ }\\ 
				\end{tabularx}
				\caption{\DIFaddFL{Computations corresponding to the MMFM of figure \ref{figure:AppendixMmfm1}.}}
				\label{table:MmfmComputations1}
			\end{table}
			\subsection{\DIFadd{MMFM 2}}
			...
			\begin{figure}
				...
				(3) edge node {$1:0$} (1);
				\end{tikzpicture}
				\caption{\DIFaddFL{Depiction of the MMFM corresponding to table \ref{table:MmfmComputations2}.}}
				\label{figure:AppendixMmfm2}
			\end{figure}
		\end{landscape} \DIFaddend \chapter{Matlab \DIFdelbegin \DIFdel{scripts and ProM plugins}\DIFdelend \DIFaddbegin \DIFadd{routines}\DIFaddend }
		\DIFdelbegin %DIFDELCMD < [%%%
		\DIFdel{references to the code}%DIFDELCMD < ]
		%DIFDELCMD < %%%
		\DIFdelend \DIFaddbegin \DIFadd{The relevant Matlab routines can be found in
		}\href{https://github.com/MartijnGosgens/preventive-maintenance.git}{this git repository}\DIFadd{.
