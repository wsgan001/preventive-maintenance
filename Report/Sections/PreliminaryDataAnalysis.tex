...
\added{Phase-Type distributions do have lots extra parameters but they are needed to ensure that the matrices are stochastic or proper generators. However if you relax the interpretation that there should be a proper markov chain behind the model then you get to Matrix exponential distributions. These have the same analytic form, without the nice stochastic interpretation. Here you can parametrise the distribution with the minimal number of characteristic parameters. The second argument still holds. No need to do anything with this, it just FYI.}
